...
typedef struct {
    uint8_t addr;                  /* 回路2真实物理地址 */
    uint8_t operation;             /* MBusCtrlOperation */
    uint32_t target_mask;          /* 本次需要修改的输出通道 */
    uint32_t target_value;         /* 目标值，仅target_mask覆盖的位有效 */
    uint16_t parameter_id;         /* 为后续复位/参数操作预留 */
    uint16_t parameter_value;      /* 为后续复位/参数操作预留 */
} MBusCtrlRequest;
