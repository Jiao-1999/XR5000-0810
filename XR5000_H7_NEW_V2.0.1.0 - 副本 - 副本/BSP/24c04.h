...
/* 定义读写SCL和SDA的宏，已增加代码的可移植性和可阅读性 */
	...
	#define EEPROM_I2C_SCL_1()  HAL_GPIO_WritePin(SCL_GPIO_Port, SCL_Pin, GPIO_PIN_SET)		/* SCL = 1 */
	#define EEPROM_I2C_SCL_0()  HAL_GPIO_WritePin(SCL_GPIO_Port, SCL_Pin, GPIO_PIN_RESET)
	
	#define EEPROM_I2C_SDA_1()  HAL_GPIO_WritePin(SDA_GPIO_Port, SDA_Pin, GPIO_PIN_SET);	/* SDA = 1 */
	#define EEPROM_I2C_SDA_0()  HAL_GPIO_WritePin(SDA_GPIO_Port, SDA_Pin, GPIO_PIN_RESET);		/* SDA = 0 */
	
	#define EEPROM_I2C_SDA_READ()  HAL_GPIO_ReadPin(SDA_GPIO_Port, SDA_Pin)	/* 读SDA口线状态 */
...
uint8_t ee_ReadBytes(uint16_t _usAddress,uint8_t *_pReadBuf,  uint16_t _usSize);
uint8_t ee_WriteBytes(uint16_t _usAddress,uint8_t *_pWriteBuf,  uint16_t _usSize);
void ee_Erase(void);
