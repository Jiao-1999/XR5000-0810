...
/* XR5000_W25Q_MUTEX_FIX_20260804: serialize access to the physical SPI2 Flash. */
static StaticSemaphore_t g_w25q_mutex_buffer;
static SemaphoreHandle_t g_w25q_mutex = NULL;

void W25QXX_TransactionMutexInit(void)
{
    if(g_w25q_mutex == NULL) g_w25q_mutex = xSemaphoreCreateRecursiveMutexStatic(&g_w25q_mutex_buffer);
}

void W25QXX_TransactionLock(void)
{
    if(g_w25q_mutex != NULL) (void)xSemaphoreTakeRecursive(g_w25q_mutex, portMAX_DELAY);
}

void W25QXX_TransactionUnlock(void)
{
    if(g_w25q_mutex != NULL) (void)xSemaphoreGiveRecursive(g_w25q_mutex);
}
...
void W25QXX_Write(uint8_t* pBuffer,uint32_t  WriteAddr,uint16_t NumByteToWrite)
{
    ...
    secpos    = WriteAddr / 4096; // 获取扇区起始地址
    secoff    = WriteAddr % 4096; // 在扇区内的偏移
    secremain = 4096 - secoff;    // 扇区剩余空间大小

    if(NumByteToWrite <= secremain) // 如果待写入字节小于剩余字节
		{
			secremain = NumByteToWrite; // 把写入字节赋值给剩余字节
		}
    while(1)
    {
       ...
       {
           if(W25QXX_BUF[secoff + i] != 0XFF) // 如果剩余地址中有不为0xFF的表明下面有数据
					 {
						 break;//需要擦除
					 }
       }
...
// 每次从头开始写入
void BspFlashWrite(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
	uint32_t secpos;
	uint16_t secoff;
	uint16_t secremain;
