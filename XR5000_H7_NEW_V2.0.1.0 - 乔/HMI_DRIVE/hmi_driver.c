...
#define TX_8_2(P2) SEND_DATA_2((P2)&0xFF)                    //发送单个字节
#define TX_8N_2(P2,N2) SendNU8_2((uint8 *)P2,N2)                 //发送N个字节
#define TX_16_2(P2) TX_8_2((P2)>>8);TX_8_2(P2)                 //发送16位整数
#define TX_16N_2(P2,N2) SendNU16_2((uint16 *)P2,N2)              //发送N个16位整数
#define TX_32_2(P2) TX_16_2((P2)>>16);TX_16_2((P2)&0xFFFF)     //发送32位整数
...
#define BEGIN_CMD() HmiTxFrameBegin();TX_8(0XEE)            //帧头
#define END_CMD() TX_32(0XFFFCFFFF);HmiTxFrameEnd()       //帧尾

#define SEND_DATA_2(P2) SendChar_2(P2)          //发送一个字节
#define BEGIN_CMD_2() HmiTxFrameBegin();TX_8_2(0XEE)            //帧头
#define END_CMD_2() TX_32_2(0XFFFCFFFF);HmiTxFrameEnd()       //帧尾
...
void SendNU8_2(uint8 *pData,uint16 nDataLen)
{
    uint16 i = 0;
    for (;i<nDataLen;++i)
    {
...
void SendNU16_2(uint16 *pData,uint16 nDataLen)
{
    uint16 i = 0;
    for (;i<nDataLen;++i)
    {
...
*  \brief  发送握手命令
*/
//系统复位指令 EE 07 35 5A 53 A5 FF FC FF FF
void SysReste()
{
    BEGIN_CMD();
    TX_8(0x07);
...
void SetScreen2(uint16 screen_id)
{
    BEGIN_CMD_2();
    TX_8_2(0xB1);
    TX_8_2(0x00);
...
void SetTextValue2(uint16 screen_id,uint16 control_id,uchar *str)
{
    BEGIN_CMD_2();
    TX_8_2(0xB1);
    TX_8_2(0x10);
...
void SetTextInt32_2(uint16 screen_id,uint16 control_id,uint32 value,uint8 sign,uint8 fill_zero)
{
    BEGIN_CMD_2();
    TX_8_2(0xB1);
    TX_8_2(0x07);
    TX_16_2(screen_id);
    TX_16_2(control_id);
    TX_8_2(sign?0X01:0X00);
    TX_8_2((fill_zero&0x0f)|0x80);
    TX_32_2(value);
    END_CMD_2();
}
/*! 
...
*  \param  control_id 控件ID
*/
void setkey_Value(uint16 screen_id,uint16 control_id,uint8_t status)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x10);
...
void SetControlForeColor2(uint16 screen_id,uint16 control_id,uint16 color)
{
    BEGIN_CMD_2();
    TX_8_2(0xB1);
    TX_8_2(0x19);
...
*  \brief     设置屏幕RTC时间
*/
void set_RTC(uint8_t niansz,uint8_t yuesz,uint8_t risz,uint8_t shisz,uint8_t fensz,uint8_t miaosz)
{
		uint8_t nian1,yue1,ri1,shi1,fen1,miao1;
		nian1=RTC_TEN_TO_BCD(niansz);
		yue1=RTC_TEN_TO_BCD(yuesz);
		ri1=RTC_TEN_TO_BCD(risz);
		shi1=RTC_TEN_TO_BCD(shisz);
		fen1=RTC_TEN_TO_BCD(fensz);
		miao1=RTC_TEN_TO_BCD(miaosz);
    BEGIN_CMD();
    TX_8(0x81);
		TX_8(miao1);//秒
		TX_8(fen1);//分
