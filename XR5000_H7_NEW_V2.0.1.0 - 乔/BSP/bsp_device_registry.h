...
/* 设备通用能力。这里只描述“设备能做什么”，具体Modbus报文仍由所属回路驱动实现。 */
#define DEVICE_CAP_STATUS_READ       (1UL << 0) /* 支持状态读取 */
#define DEVICE_CAP_OUTPUT_CONTROL    (1UL << 1) /* 支持输出控制 */
#define DEVICE_CAP_EVENT_RECEIVE     (1UL << 2) /* 支持接收火警/故障事件 */

/* 通用输出通道位。产品驱动负责解释每一位对应的实际输出。 */
...
typedef struct
{
    ...
    uint32_t supported_outputs;    /* 支持的输出通道位；非输出设备为0 */
    uint8_t control_driver;        /* DeviceControlDriver；不在产品表保存具体协议细节 */
...
uint32_t DeviceRegistry_GetCapabilities(uint16_t product_code); /* 获取设备通用能力位 */
uint32_t DeviceRegistry_GetSupportedOutputs(uint16_t product_code); /* 获取设备支持的输出通道位 */
uint8_t DeviceRegistry_GetControlDriver(uint16_t product_code); /* 获取设备控制驱动编号 */
