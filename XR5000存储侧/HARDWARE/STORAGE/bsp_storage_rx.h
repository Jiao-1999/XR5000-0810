 ...
 * [GB4717 表B.2] 本结构即表B.2"数据信息"字段的物理落盘格式,
 *   逐字节对应: 控制器号/部件号/设备号/通道号/类型/事件/状态/年月日时分秒
 *============================================================*/
#pragma pack(push, 1)
typedef struct {
 ...
 * [GB4717 B.1.2.2] 首火警/火灾报警/故障三类信息独立记录且不被其他信息覆盖:
 *   Z0首警/Z1火警/Z2故障 三个独立分区物理隔离, Z3通用区无法侵占前三个区。
 * [GB4717 B.1.2.1] 存储容量(区域型): 首警61440条/火警122880条/故障122880条/通用约664960条。
 * [GB4717 表B.2] EventRecord_t 17字节即表B.2"数据信息"物理落盘格式,
 *   导出时经 gb4717_export.c 原样透传, 满足 B.1.3.1"导出格式与存储格式相同"。
 * [GB4717 B.1.4.4] W25Q256为非易失Flash, 断电后记录保持不丢失;
 *   写指针经 meta A/B 双库持久化, 掉电恢复见 StorageRx_MetaLoad()。
 * W25Q256 32MB布局:
