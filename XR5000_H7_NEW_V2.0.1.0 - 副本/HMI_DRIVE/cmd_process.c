...
#include "bsp_storage_event.h"  /* 黑匣子存储事件记录(GB4717-2024附录B) */
#include "bsp_fecbus_report.h" /* FECbus RS485 事件上报模块 (GB4717 附录C) */
...
void NotifyScreen(uint16 screen_id)
{
    ...
    AlarmHistoryFilter_NotifyScreen(screen_id);
...
{
	AlarmHistoryFilter_NotifyButton(screen_id, control_id, state);
...
{
   AlarmHistoryFilter_NotifyText(screen_id, control_id, str);
   { 
...
{
  AlarmHistoryFilter_NotifyMenu(screen_id, control_id, item, state);
	...
	for(uint8_t addr = 1; addr < RS485_DETECT_MAX_DEVICES; addr++)
	{
		...
		if(type == RS485_DETECT_TYPE_XR805 || type == RS485_DETECT_TYPE_XR8303 || type == RS485_DETECT_TYPE_XR8305)
		{
			...
			if(old_h2 != h2_state)
			{
                ...
                if(old_h2 == 8U) StorageEvent_LogFault(addr, DEV_TYPE_H2, 3, 0, 1);
                if(h2_state == 2U)
				{
					getBM8563TimeToSystemTime();
                ...
                else if(h2_state == 8U) { RS485Loop3AddFault(addr, RS485_LOOP3_FAULT_H2, RS485_H2_SENSOR_FAULT); StorageEvent_LogFault(addr, DEV_TYPE_H2, 3, 0, 0); }
        ...
        if(old_state != raw_state)
        {
            if(type == 6U)
            {
                ...
                else if(raw_state == 8U)
                {
                    ...
                    FecbusReport_Fault(addr, DEV_TYPE_TEMPERATURE, 1, 0, 0); /* FECbus:温度传感器故障 */
                }
            }
            else
            {
