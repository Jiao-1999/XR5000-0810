...
#include "bsp_storage_event.h"  /* ��ϻ�Ӵ洢�¼���¼(GB4717-2024��¼B) */
#include "bsp_fecbus_report.h" /* FECbus RS485 �¼��ϱ�ģ�� (GB4717 ��¼C) */
...
uint32_t yonghumima=0,yonghumm1=0,yonghumm2=0,yonghumm3=0,mmsdSTA=0;//�û�����
...
typedef enum
{
	OUT_FIRE_NO_START  = 1,  // ���װ��δ����
	OUT_FIRE_SUSPEND   = 2,  // ���װ��ֹͣ����

	SPRAY_START_DELAY  = 3,  // ��ŵ���ʱ
	SPRAY_INTERVAL_T1  = 4,  // ��� �ȴ���2�����
	SPRAY_SECOND_DELAY = 5,  // ��2����ŵ���ʱ״̬
	SPRAY_INTERVAL_T2  = 6,  // ��� �ȴ���3�����
	SPRAY_THIRD_DELAY  = 7,  // ��3����ŵ���ʱ״̬
}OutFireDeviceState; // ���װ��״̬


typedef struct
{
	uint8_t cluster_id; // ��id
	uint8_t pack_id;    // ���ڶ�Ӧ���µ�pack id
	
	uint8_t cabin_id;   // ��id

	int8_t lunch_state; // ��Ϊ�з����� ������¼��ֹ��� -1:��ֹ -2:��������ֹ 0:δ���� 99:��������ʱ�ڼ���ֹ

	AlarmTimeRecord atr;

}PackAlarmStorage;    // ���ڸýṹ���������������ѭ����ʾ

PackAlarmStorage pas[224] = { 0 };

uint8_t pas_pointer = 0;
uint8_t last_pas_len = 0;
uint8_t pas_fresh_point = 0;// ��Ļ��ʾˢ��ָ��

uint8_t multiple_alarm_fresh_flag = 0;
uint8_t pas_traverse_pointer = 1;

uint8_t alarm_number = 0; // ��������
uint8_t last_alarm_num = 255; // Ĭ��ִ��һ��
...
typedef struct
{
	...
	uint8_t cluster_id;     // ��ID
	uint8_t pack_id;        // ��ID
}DetectorAddrAttribute;   // ԭʼ�������ͣ����ֲ֣�������

typedef struct
{
	uint8_t temperature_type; // �¶�
	uint8_t carbon_type;      // һ����̼
	uint8_t smoke_type;       // ����
}CabinAlarmType;     //��̽�����ı�������
...
uint8_t fire_alarm_start_index = 0; // ������Ϣ�������ݵ���ʼ����
...
// Ԥ���洢 ��Ԥ��ָ ���п�ȼ����
typedef struct {
	uint8_t self_bottom_point;       // ��������ĵ�ָ��
	
	uint8_t point_history_len;       // ��¼��ʷָ�볤��

	uint8_t detector_class[224];     // ̽��������
	
	uint8_t alarm_type[224];				 // ����������
	
	DetectorAddrAttribute da[224];   // ̽��������

	AlarmTimeRecord atr[224];        // ����ʱ���¼
	
	uint16_t fresh_time_count;        // ������ѯ��ʾ
}PackCabinForeWarnStorage;  // �ְ���������Ԥ���洢
...
typedef struct {
	...
	uint16_t fresh_time_count;       // ��ѯ��ʾ��ʱ
...
typedef struct
{
	uint8_t detector_class; // ̽��������
	
	DetectorAddrAttribute da;   // ̽��������
	
	AlarmTimeRecord atr;        // ����ʱ���¼
	uint8_t fault_type;
	
}PackCabinFaultStorage;   // �����洢���� ������Ϣ	
uint8_t pcfs_fresh_ctrl = 255;    // ˢ�¿���ָ�� ��ʼ��ֵΪ���ֵ
uint8_t pcfs_buttom_point = 0;  // βָ�� ��¼���ݳ���
PackCabinFaultStorage pcfs[224] = {0}; // �������й�����Ϣ
...
FireAlarmStorage fire_alarm_flag = {0}; // ȫ����ʼ��Ϊ0

typedef struct
{
	uint8_t fire_alarm_id_buff[300]; // �洢�������
	uint8_t faib_buttom_point;      // fire_alarm_id_buff����ָ��
	uint8_t storage_pas_len;        // �洢�𾯼�¼���� ���Ȳ�һ��ʱ��������
}FireAlarmNumRecord; // �ѻ𾯱����ı��ɸѡ����
...
typedef enum
{
	FIRE_EXTINGUISH_FORCE_START    = -4,    // �ֶ�ǿ������
	FIRE_EXTINGUISH_RESTART_FINISH = -3,    // ����������ֵΪ��״̬ �����´�����
	FIRE_EXTINGUISH_CAN_RESTART    = -2,    // ��������������״̬
	FIRE_EXTINGUISH_FORCE_STOP     = -1,    // ǿ�ƽ���
	
	FIRE_EXTINGUISH_MODE_JUDGEMENT       = 0,      // �жϵ�ǰ״̬
	FIRE_EXTINGUISH_START_SPRAY_DELAY    = 1,      // ������ʱ
	FED_START_SPRAY_DELAY_FINISH_FLAG    = 2,      // ���װ����������ʱ ������־
	FIRE_EXTINGUISH_FIRST_SPRAY_START    = 3,      // ��һ����ſ�ʼ
	FIRE_EXTINGUISH_FIRST_SPRAY_FINISH   = 4,      // ��һ��������
	FIRE_EXTINGUISH_SECOND_SPRAY_DELAY   = 5,      // �ڶ������������ʱ
	FED_SECOND_SPRAY_DELAY_FINISH_FLAG   = 6,      // ���װ�õڶ�����������ʱ ������־
	FIRE_EXTINGUISH_SECOND_SPRAY_START   = 7,      // �ڶ�����ſ�ʼ
	FIRE_EXTINGUISH_SECOND_SPRAY_FINISH  = 8,      // �ڶ���������
	FIRE_EXTINGUISH_THIRD_SPRAY_DELAY    = 9,      // ���������������ʱ
	FED_THIRD_SPRAY_DELAY_FINISH_FLAG    = 10,     // ��������ŵ���ʱ������־
	FIRE_EXTINGUISH_THIRD_SPRAY_START    = 11,     // ��������ſ�ʼ
	FIRE_EXTINGUISH_THIRD_SPRAY_FINISH   = 12,     // ������������
	FIRE_EXTINGUISH_ALL_SPRAY_COMPLETE   = 13,     // ȫ��������

	FIRE_EXTINGUISH_CLUSTER_VALVE_OPEN   = 14, // �ص�ŷ���
	FIRE_EXTINGUISH_CABIN_VALVE_OPEN     = 15, // �ֵ�ŷ���
	FIRE_EXTINGUISH_CYLINDEF_1_OPENED    = 16, // ��ƿ1��ŷ���
	FIRE_EXTINGUISH_CYLINDEF_2_OPENED    = 17, // ��ƿ2��ŷ���
	
	FIRE_EXTINGUISH_STARYUP_FINISH_FLAG  = 18, // ���װ���������뵹��ʱ
	
	FEEDBACK_1_PRESS = 19,
	FEEDBACK_2_PRESS = 20,
	
}FireExtinguishDeviceActionType; // ���װ�ö�������

// ����װ�ü�¼
typedef struct
{
	uint8_t cluster_id[224];   // ��id
	uint8_t pack_id[224];      // ���ڶ�Ӧ���µ�pack id
	
	uint8_t cabin_id[224];     // ��id
	
	AlarmTimeRecord atr[224];  // ����ʱ���¼
	
	int8_t fed_action[224];   // ���װ�ö�����¼
	
	uint8_t self_point_len;    // 
	uint8_t last_point_len;    // ����ָ��
	
	uint16_t countdown_val[224];    // ����ʱ��ֵ 
	uint16_t start_cntd_time[224];  // ��ʼ����ʱ��ʱ���
	uint16_t curr_cntd_time[224];   // ��ǰϵͳʱ�� ������ʾ����ʱ
}FireExtinguishDeviceActionSave; // ���װ�ö�����¼
...
typedef struct
{
	uint8_t  curr_detector_page; // ��ǰ��ʾҳ
	uint8_t  last_detector_page; // ��һ����ʾҳ
	
	uint8_t  detector_offline_fresh_flag[11];
	
	uint8_t  detect_online_state[21][11];   // ����״̬
	uint8_t  detect_shield_state[21][11];   // ����״̬
	
	uint8_t  last_temperat_state[21][11]; // ��һ���¶�״̬
	uint8_t  last_temperature[21][11];   // ��һ�ε��¶�
	
	uint8_t  last_smoke_state[21][11];   // ��һ������״̬
	
	uint8_t  last_co_state[21][11];      // 
	uint16_t last_co_concentrat[21][11]; // ��һ�ο�ȼ�����Ũ��
...
typedef struct
{
	uint8_t force_fresh_flag;    // ǿ��ˢ�±�־λ
	
	uint8_t curr_detector_page;  // ��ǰҳ
	uint8_t last_detector_page;  // ֮ǰҳ
	
	uint8_t curr_pack_id;
	
	uint8_t last_temperat_state; // ��һ���¶�״̬
	uint8_t last_temperature;    // ��һ�ε��¶�
	
	uint8_t last_smoke_state;   // ��һ������״̬
	
	uint8_t last_co_state;      // 
	
	uint8_t lat_detector_online_num[4]; // ��ʷ�������� 
	
	uint8_t last_derector_state[3][32];
	
	uint16_t last_co_concentrat; // ��һ�ο�ȼ�����Ũ��
...
typedef struct
{
	...
	uint8_t last_temperat_state; // ��һ���¶�״̬
	uint8_t last_temperat_value; // ��һ�ε��¶�
...
typedef struct
{
	uint8_t curr_partition; // ��ǰ���ĸ�����
	uint8_t last_partition; // ��������
	uint8_t curr_point_site[4]; // ��ǰ��ͷ����һ��
	uint8_t last_point_site[4]; // ��������
	
	uint8_t last_show_len[4]; // �洢��һ��ˢ�µ�λ�� �����ڿɻָ��ĵط�ˢ�¼�ͷλ��
	
	uint8_t curr_menu_state; // ��ǰ�˵�״̬
	uint8_t last_menu_state; // ��һ�β˵�״̬
...
typedef struct
{
	uint8_t curr_page[4]; // ��ǰҳ 
	
	uint16_t record_sum[4]; // �����ͼ�¼����
	
	uint8_t force_fresh_flag; // ǿ��ˢ�±�־
	
	uint8_t curr_show_type; // ��ǰ��ʾ����
...
uint8_t PCAC_zxwz_buf[12]={0,4,16,28,40,52,64,76,88,100,112,0};//����״̬�ؼ�ID
uint8_t PCAC_wdwz_buf[12]={0,9,21,33,45,57,69,81,93,105,117,0};//�¶ȿؼ�ID
uint8_t PCAC_ywtb_buf[12]={0,12,24,36,48,60,72,84,96,108,120,0};//����״̬�ؼ�ID
uint8_t PCAC_cotb_buf[12]={0,11,23,35,47,59,71,83,95,107,119,0};//CO״̬�ؼ�ID
uint8_t PCAC_ch4yb_buf[12]={0,13,25,37,49,61,73,85,97,109,121,0};//CH4״̬�ؼ�ID

uint8_t cang_zxwz_buf[8]={0,16,20,36,52,72,88,0};//����״̬�ؼ�ID

uint8_t cang_wdwz_buf[8]={0,10,17,41,49,77,111,0};//�¶ȿؼ�ID
//uint8_t cang_ywtb_buf[8]={0,12,19,35,51,71,87,0};//����״̬�ؼ�ID
//uint8_t cang_cotb_buf[8]={0,13,25,41,61,77,93,0};//CO״̬�ؼ�ID
//uint8_t cang_ch4yb_buf[8]={0,14,26,42,62,78,94,0};//CH4״̬�ؼ�ID
//uint8_t cang_vocyb_buf[8]={0,15,27,43,63,79,95,0};//VOC״̬�ؼ�ID

uint8_t cang_cowb_buf[8]={0,117,25,122,61,89,138,0};//CO״̬�ؼ�ID
uint8_t cang_h2wb_buf[8]={0,118,26,124,62,90,139,0};//H2״̬�ؼ�ID
uint8_t cang_ch4wb_buf[8]={0,12,33,127,69,93,142,0};//CH4״̬�ؼ�ID
uint8_t cang_ywwb_buf[8]={0,120,35,129,71,95,144,0};//YW״̬�ؼ�ID
uint8_t cang_vocwb_buf[8]={0,15,34,128,70,94,143,0};//VOC״̬�ؼ�ID

uint8_t cang_XH_buf[8]={0,149,150,151,152,153,154,0};//�ͺſؼ�ID
...
uint8_t BJ_packjiyibuf_yw[30][PACK_NUM_BACKUP];//2024-03-09���ӣ���ֹ�¶�����Ԥ������ʱ���ظ���¼
uint8_t BJ_packjiyibuf_co[30][PACK_NUM_BACKUP];//2024-03-09���ӣ���ֹ�¶�����Ԥ������ʱ���ظ���¼
uint8_t BJ_packjiyibuf_ch4[30][PACK_NUM_BACKUP];//2024-03-09���ӣ���ֹ�¶�����Ԥ������ʱ���ظ���¼
...
// 2025/11/17 18:10 ���� ѭ����ʾÿһ��̽������״̬ ��������ѯ �����·�ҳ �������в� ͳһ��Ϊ xx��·xxID̽����

typedef struct
{
	uint8_t poll_circuits_id; // ��·ID
	uint8_t poll_detector_id; // ̽����ID
	
	// �˴�����Ϊ�����Ƹ��� ����Ϊ��ֻ��ʾ���ߵ�̽���� ����ɸѡID
	uint8_t last_circuits_id; // �ϴλ�·ID
	uint8_t last_detector_id; // �ϴ�̽����ID
	
	uint8_t poll_temper_value; // ��ѯ�¶�ֵ
	uint8_t poll_smokes_state; // ��ѯ����״̬
	
	uint16_t poll_carbon_value; // ��ѯһ����ֵ̼
	
	uint16_t poll_hydrog_value; // ��ѯ����ֵ ʡ��en
	
	uint8_t poll_detect_name;  // ��ѯ ̽��������
	
	uint8_t poll_sensor_state; // ̽��������״̬
	
	uint8_t key_perss_fresh; // 'n'��һ�� 'p'��һ��
}PollingShowBase_t; // ��ѯ��ʾ���ṹ��

typedef struct
{
	// ����ID����
	uint8_t verb_circuits_id; // ��·ID
	uint8_t verb_detector_id; // ̽����ID
	uint8_t last_detector_id; // ��һ�β�ѯ��̽����ID �������Ƹ���
	
	uint8_t force_fresh_ctrl; // ����ID��ǿ��ˢ��һ��
	
	uint8_t verb_temper_value; // �¶�ֵ
	uint8_t last_temper_value; // �¶�ֵ
	
	uint8_t verb_smokes_state; // ����״̬
	uint8_t last_smokes_state; // ����״̬
	
	uint16_t verb_carbon_value; // һ����ֵ̼
	uint16_t last_carbon_value; // һ����ֵ̼
	
	uint16_t verb_hydrog_value; // ��ѯ����ֵ ʡ��en
	uint16_t lsat_hydrog_value; // ��ѯ����ֵ ʡ��en
	
	uint8_t verb_detect_name;  // ��ѯ ̽��������
	uint8_t lsat_detect_name;  // ��ѯ ̽��������
	
	uint8_t verb_sensor_state; // ̽��������״̬
	uint8_t last_sensor_state; // ̽��������״̬
	
}InqueryShowBase_t; // ������ѯ���ṹ��
...
// ������¼IO������й�����Ϣ 2025/11/21 13:49

// ����ģ�⴮�����ֽ������
typedef struct
{
	uint8_t serial_port_state; // ���ڴ�״̬
	
	uint8_t serial_port_comid; // ����ѡ��˿�ID ѡ������ĸ����������д���
	
	uint8_t serial_port_send_mode; // 0:�ַ��� 1:16����
	uint8_t serial_port_show_offset;
	
	uint8_t serial_port_show_mode; // 0:�ַ��� 1:16����    
	
	uint8_t serial_port_send_len; // �ӷ��ͻ������л�ȡ���ַ�����
	
	uint8_t serial_port_send_new_row; // 0:���������� 1:��������
	
	uint8_t serial_port_send_buff[256]; // ���ͻ�����
...
void FaultDataInit(PackCabinFaultStorage *pcfs_entry)
{
	...
	pcfs_fresh_ctrl = 255;    // ˢ�¿���ָ�� ��ʼ��ֵΪ���ֵ
	pcfs_buttom_point = 0;  // βָ�� ��¼���ݳ���
}
...
{
	alarm_number = 0; // ��ʼ����������
	last_alarm_num = 255; // ��ʼ������������������
	last_online_detector_num = 255; // ��ʼ������̽����������������
	last_disconnect_detector_num = 255;	 // ��ʼ������̽����������������
	ds.last_num = 255;
	ds.curr_num = 0;
}
...
{
	FaultDataInit(pcfs); // ������ϼ�¼
	ForeAlarmDataInit(&pcfws); // ���Ԥ����¼
	FireAlarmDataInit(&pcfas); // ����𾯼�¼
	FireExtinguishDataInit(&fedas); // ������������¼
	PackAndCabinHistoryAlarmInit(pas);
	ClearDetectorHistoryData(); // ���̽������ʷ��¼
...
static void CreatNewFireExtinguishRecord(
	FireExtinguishDeviceActionSave *fedas_entry, // Ĭ�ϸ�ֵ�Ľṹ��
	FireExtinguishDeviceActionSave *copy_fedas,  // Ĭ�ϸ�ֵ�Ľṹ��
	uint8_t copy_dedas_offset,
	FireExtinguishDeviceActionType state, 
	uint16_t state_switch_delay             // ״̬�л���ʱ 
);
...
//uint8_t license_allow_use_state = 0; // Ĭ�Ͻ���
uint8_t license_allow_use_state = 1; // ʼ�տ������������
...
static void FormatDetectorText(uint8_t circuit, uint8_t addr, uint8_t *buf)
{
	...
	case 3:
	{
		...
		p += sprintf(p, "��%d��·  ����̽����%d��  ", circuit, addr);

		if (enable & (1 << 5))
		{
			int16_t temp = RS485Detect_GetTemperature(addr);
			p += sprintf(p, "�¶ȣ�%d��  ", temp);
		}
		if (enable & (1 << 0))
		{
...
static const char* GetMBusDeviceChineseName(uint8_t addr)
{
	...
	switch (type)
	{
		case MBUS_CONTROL_DEV_SGBJQ:  return "���ⱨ����";
		case MBUS_CONTROL_DEV_XR2200: return "�ֶ�������";
		case MBUS_CONTROL_DEV_FIRE_DISPLAY: return "������ʾ��";
		default: return "δ֪�豸";
	}
}
...
static uint8_t FormatRS485DetectFireExtinguisherLine(uint8_t *buf, uint8_t sequence, FireExtinguishDeviceActionSave *fedas_entry, uint8_t data_index, uint16_t temp_time)
{
	...
	switch(fedas_entry->fed_action[data_index])
	{
		case FIRE_EXTINGUISH_MODE_JUDGEMENT:
			...
			{
				sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���ֶ��������װ��", sequence,
		...
		case FIRE_EXTINGUISH_START_SPRAY_DELAY:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ����������ʱ%d", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
			break;
		case FED_START_SPRAY_DELAY_FINISH_FLAG:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�õ�1���������", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		...
		case FIRE_EXTINGUISH_THIRD_SPRAY_START:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�����ʣ��ʱ��%d", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
			break;
		...
		case FIRE_EXTINGUISH_SECOND_SPRAY_DELAY:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�õ�2����������ʱ%d", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
			break;
		case FED_SECOND_SPRAY_DELAY_FINISH_FLAG:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�õ�2���������", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		...
		case FIRE_EXTINGUISH_THIRD_SPRAY_DELAY:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�õ�3����������ʱ%d", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
			break;
		case FED_THIRD_SPRAY_DELAY_FINISH_FLAG:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ�õ�3���������", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		...
		case FIRE_EXTINGUISH_ALL_SPRAY_COMPLETE:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ��������", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		...
		case FIRE_EXTINGUISH_CYLINDEF_1_OPENED:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ��1����", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		case FIRE_EXTINGUISH_CYLINDEF_2_OPENED:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ��2����", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		case FIRE_EXTINGUISH_FORCE_STOP:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ����������ʱ--", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
		...
		case FIRE_EXTINGUISH_RESTART_FINISH:
			sprintf((char*)buf, "%03d %d/%02d/%02d %02d:%02d:%02d ��3��· %d�� �� ���װ���ֶ�ֹͣ����", sequence,
				fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
				fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
				fedas_entry->pack_id[data_index]);
			break;
...
static void FormatScreen69DetectorText(uint8_t circuit, uint8_t addr, uint8_t *buf)
{
	...
	switch (circuit)
	{
		...
		case 3:
		{
			...
			n = snprintf(p, remain, "%02d%03d ����̽����", circuit, addr);
			p += n;
			remain -= n;
			...
			{
				uint16_t pressure = RS485Detect_GetSensorValue(addr, RS485_SENSOR_PRESSURE);
				snprintf(p, remain, " ѹ��:%dhPa", pressure);
			}
			{
		...
		default:
			snprintf((char *)buf, 128, "%02d%03d ̽����", circuit, addr);
			break;
	}
}
    ...
    case NOTIFY_CONTROL:
        {
            if(ctrl_msg==MSG_GET_CURRENT_SCREEN)                                    //����ID�仯֪ͨ
            {
                NotifyScreen(screen_id);                                            //�����л������ĺ���
						...
						if(ctrl_msg==TUBIAO_shangchuan)                                    //����ID�仯֪ͨ
            {
               TB_sahngchuan(screen_id,control_id,control_type,msg->param[0]);                      //ͼ��ؼ��ϴ������ĺ���
            }else
            {
                switch(control_type)
                {
...
void NotifyScreen(uint16 screen_id)
{
    ...
    DeviceThreshold_NotifyScreen(screen_id); //�ڹ��������п��������л�֪ͨ����¼��ǰ����ID
    ...
    if(screen_id == 1)
    {
			SetTextValue(1, 11, (uint8_t *)"FGS-XR5000.��������������");//ˢ����������
			
			SetTextInt32(1, 30, SystemSaveInfo.slave_addr485_Station,0,1); // 30�ǳ�վ��
			...
			SetTextInt32(1, 8, alarm_number,0,1);  //����������ʾ
		...
		else if(screen_id == 7) // 
		{
			...
			uint8_t temp_buff[8] = {0};
			sprintf((char *)temp_buff, "��·%d", pack_circuit);
			SetTextValue(7, 231, temp_buff); // ˢ�»�·����
			switch(pack_circuit)
			{
				case 1:
			...
			// {
			// 	temp_buff[0] = pack_online_buff[pack_circuit][i + 1] ? 1 : 0;
			// 	AnimationPlayFrame(7, i, temp_buff[0]);//(����ID,�ؼ�ID,֡ID) 0�죬1��
			// }
		}
		else if(current_screen_id == 10)
    {
			...
			{
				SetTextValue(10, 9, "��ǰԤ������֤:��");
			}
			clearTextValue(10, 7);
			
			SetTextValue(10, 17, "�ϴ���Ч����֤:");
			SetTextValue(10, 18, SystemSaveInfo.last_license_store);
			
			SetTextValue(10, 1, "��ǰ��������֤:");
			...
			sprintf((char *)buff, "��Чʱ��:%d/%d/%d %d:%d:%d", SystemSaveInfo.license_year, SystemSaveInfo.license_month, SystemSaveInfo.license_days, 
				SystemSaveInfo.license_hour, SystemSaveInfo.license_minute, SystemSaveInfo.license_second);
			SetTextValue(10, 20, buff);
			
			sprintf((char *)buff, "ʣ��ʱ��:%d", remain_use_time);
    ...
    {
			clearTextValue(19,2);//(����ID,�ؼ�ID��	
			SetTextValue(19,2,"��λϵͳ��");
		}
		...
		else if(screen_id==22)  //                                        
    {
		...
		{
			clearTextValue(23,2);//(����ID,�ؼ�ID��
		}
		else if(screen_id == 24)
		{
			clearTextValue(24,2);//(����ID,�ؼ�ID��	
			clearTextValue(24,5);//(����ID,�ؼ�ID��
			clearTextValue(24,6);//(����ID,�ؼ�ID��
			clearTextValue(24,7);//(����ID,�ؼ�ID��
		}
		...
		else if(current_screen_id == 27)
    {
			...
			else if(SystemSaveInfo.license_remain_day != 999)
			{
				...
				sprintf((char *)slicense_buff, "����״̬:ʣ��%d��", remain_use_time);
				SetTextValue(27, 7, slicense_buff);
			}
			else
			{
				SetTextValue(27, 7, "����״̬:������Ч");
			}
		...
		else if(screen_id == 41)
		{
			...
			// ��ȡһ��ʱ��
			BM8563_Soft_I2C_GetTime(&SystemTime); // ��һ��ʱ�� ��ŵ�ȫ�ֽṹ����
		...
		else if(screen_id == 59)
		{
			...
			{
				clearTextValue(59 , i);//(����ID,�ؼ�ID)
			}
			clearTextValue(59 , 2);//(����ID,�ؼ�ID)
		}
		...
		else if(screen_id == 69)
		{
			...
			sprintf((char *)temp_buff, "��ǰ��ʾ���� %d ��·", screen69_circuit);
			SetTextValue(current_screen_id, 200, temp_buff);
			sprintf((char *)temp_buff, "ע:�˽������ʾ�����߲�����̽����");
			SetTextValue(69, 400, temp_buff);
			sprintf((char *)temp_buff, "��һҳ");
...
uint32_t last_time_stamp = -3600000; // �ϵ�����һ��ʣ��ʱ��ʱ��
/*! 
*  \brief  ��������
*/ 
/* XR5000_SCREEN69_FLICKER_FIX_20260727: ����69����ṹ�壬��ԭʼ����ֵ�Ƚ����strcmp�ַ����Ƚϣ�����Ƶ�� */
typedef struct {
	uint16_t temper_val;
	uint16_t co_val;
	uint16_t h2_val;
...
void UpdateUI(void)
{
	...
	if(check_record_pending != 0U)
	{
		...
		StorageEvent_LogCheckButton(); /* ��ϻ��:��鹦�ܰ�ť����(EVT_CHECK_BUTTON=129), ����ȥ�ػ���ֻ��һ�� */
	}
	...
	if(tim_get == 10)
	{
		...
		multiple_alarm_fresh_flag = 1; // ˢ�±�־λ��һ
		fed_fresh_flag = 1; // ÿ��һˢ
	...
	if(curr_time_stamp - last_time_stamp >= 3600000)
	{
		...
		if(SystemSaveInfo.license_remain_day != 6666 && SystemSaveInfo.license_remain_day != 999)
		{
			...
			getBM8563TimeToSystemTime(); // ��ȡһ��RTCʱ��
			...
			{
//				license_allow_use_state = 0; //��Ĭ��ֵΪ1��ʱ������ע�͵�
				remain_use_time = 0;
			}
			...
			if(remain_use_time < 4) // 
			{
				...
				sprintf((char *)slicense_buff, "����:����ʣ��ʹ������ֻʣ��%d��,�������Ͻ�ͼ���ȡ������Ϣ", remain_use_time);
	...
	if(kaijiyanshi >= ONLINE_TIMEOUT)//������ʱONLINE_TIMEOUT���ż����������
	{
		// ��ȡ�������� ���������������� ���ṹ�帳ֵ
		pack_disconnect_sum = ClusterPackDataDeal_Plus(pcfs, &pcfs_buttom_point); 
		// ��ȡ�ֵ����� �������ֱ������� �洢��������Ϣ�� Ŀǰ��û���ͼ� û�����ӱ�����Ϣ 2025/07/09
		cabin_disconnect_sum = CabinDataDeal(pcfs, &pcfs_buttom_point); 
		//
		point_type_disconnect_sum = PointTypeDetectorDataDeal(pcfs, &pcfs_buttom_point); // 2025/11/17 10:27 ���ӵ��Ͷ�����̽����
		...
		/* ���ܵ���������IG3306��4·����24V�����⣻ʱ�䣺2026-08-06 */
		// �ж��Ƿ��е��� ���Ϲ��ϼ̵����������ӶԻ�·����485̽���·�Ĺ����ж�
...
//		}
		...
		alarm_number = pcfas.self_bottom_point + pcfws.self_bottom_point; // ����������ֵ

		/*
		// Ŀǰ����ʾԤ�� �� ͳһ��ʾ���� �˴�ֻ�������𾯱�� ִ�������豸����
		if(pas_pointer != 0) { // ����𾯼�¼��Ϊ��
			fire_alarm_state = 1; // ������ָʾ��
		...
		*/
		...
		for(uint8_t i = 0; i < 12; i++)
		{
			if(BMS_Temp[i]>=3 && BMS_BJ[i]==0)
			{
				...
				SetTextValue(1,4,"BMS����״̬����1�ص�ظ��£�");//ˢ�±�������
...
//		// ����������� ��ʱû�м�¼�������ֹͣʱ��
		...
		{
			screen_show_siren_information |= 0xF0; // ���ִ�й� 
			// ��¼���𾯷����� �������� ����FLASH��ǰ������һ�λ�ȡRTC����
...
//			StoragePackCabinForeWarn(&pcfws, LINKAGE_CLUSTER_ID, ALARM_ANNUNCIATOR_ID, AlarmCtrlKey);
			// ������ֱ������� 
			StoragePackFireAlarm(&pcfas, LINKAGE_CLUSTER_ID, ALARM_ANNUNCIATOR_ID, AlarmCtrlKey); // ��¼�ֱ�����
			...
			silencers_state  = 0;  // ���µı��� �ر�����ָʾ��
		}
		
		if(getHandPaperState() == 0x0F) // �ֱ���Ϊ�ǻ�
		{
			// ȷ��ִֻ��һ��
	...
	{
		if(getControllorSelfCheckState() == 1) // ����Լ찴������ //��ʾ�Լ�����
		{
			// ���Լ��
			SpecialSelfCheckLedCtrl(LED_ON);
			switch(screen_fresh_num)
			{
				case 1:SetTextValue(1,4,"ϵͳ�Լ���.     ");break;   //ˢ�±�������
				case 2:SetTextValue(1,4,"ϵͳ�Լ���..    ");break;   //ˢ�±�������
				case 3:SetTextValue(1,4,"ϵͳ�Լ���...   ");break;   //ˢ�±�������
				case 4:SetTextValue(1,4,"ϵͳ�Լ���....  ");break;   //ˢ�±�������
				case 5:SetTextValue(1,4,"ϵͳ�Լ���..... ");break;   //ˢ�±�������
				case 6:SetTextValue(1,4,"ϵͳ�Լ���......");break;   //ˢ�±�������
			}
			screen_fresh_num++;
			...
			else if(show_content_delay == 10) 
			{
				...
				show_content_delay = 0; // Ŀǰֻ������ʱ��״̬��ת 
			}
			...
			switch(self_check_show_content)
			{
				case 0: {
					...
					SetTextValue(1, 12, "Ƭ�ϴ洢ϵͳ�Լ���");//
					break;
				}
				case 1:
					SetTextValue(1, 12, "ָʾ���Լ���");//
					break;
				case 2:
					SetTextValue(1, 12, "�������Լ���");//
					break;
				case 3:
					...
					break;
				case 4:
					SetTextValue(1, 12, "ϵͳ�Լ����");//
					SoundLightRelayCtrl(JDQ_OFF);
					DefauleRelayCtrl(JDQ_OFF);
					clearTextValue(1 , 12);//(����ID,�ؼ�ID)
					break;
				default:
					self_check_show_content = 0; // ��ʾ���ݸ�λ
					show_content_delay = 0; // �����ʱ Ϊ��һ����׼��
		...
		else 
		{
			if(kaijiyanshi < ONLINE_TIMEOUT)
			{
				switch(screen_fresh_num)
				{
					case 10:SetTextValue(1, 4, "ϵͳ��ʼ����.     ");break;   //ˢ�±�������
					case 20:SetTextValue(1, 4, "ϵͳ��ʼ����..    ");break;   //ˢ�±�������
					case 30:SetTextValue(1, 4, "ϵͳ��ʼ����...   ");break;   //ˢ�±�������
					case 40:SetTextValue(1, 4, "ϵͳ��ʼ����....  ");break;   //ˢ�±�������
					case 50:SetTextValue(1, 4, "ϵͳ��ʼ����..... ");break;   //ˢ�±�������
					case 60:SetTextValue(1, 4, "ϵͳ��ʼ����......");break;   //ˢ�±�������
				}
			}
			else
			{
				switch(screen_fresh_num)
				{
					case 10:SetTextValue(1, 4, "����ϵͳ������.     ");break;   //ˢ�±�������
					case 20:SetTextValue(1, 4, "����ϵͳ������..    ");break;   //ˢ�±�������
					case 30:SetTextValue(1, 4, "����ϵͳ������...   ");break;   //ˢ�±�������
					case 40:SetTextValue(1, 4, "����ϵͳ������....  ");break;   //ˢ�±�������
					case 50:SetTextValue(1, 4, "����ϵͳ������..... ");break;   //ˢ�±�������
					case 60:SetTextValue(1, 4, "����ϵͳ������......");break;   //ˢ�±�������
				}
			}
			//  �����豸����/����/���ϵ���ʾ
			...
			// XR5000_AHT20_CHANGE_20260727: display cached AHT20 temperature/humidity on home screen.
			{
				...
				if(aht20_valid)
				{
					...
					if(temp != last_aht20_temp || aht20_valid != last_aht20_valid || home_statistics_force_refresh)
					{
						...
						sprintf((char *)aht20_buff, "�¶ȣ�%d��", temp);
...
////			sprintf((char*)baojingneirong,"�豸���� �豸���:%s ��ַ����:%d",str,shebeizongshu);
//			SetTextValue(52,2,baojingneirong);//ˢ�±�������
//			
//			sprintf((char*)baojingneirong,"�������� �豸���:%s ��ַ����:%d",str,zaixianzongshu);
//			SetTextValue(52, 5, baojingneirong);//ˢ�±�������
//			
//			sprintf((char*)baojingneirong,"�������� �豸���:%s ��ַ����:%d",str,diaoxianzongshu);
//			SetTextValue(52, 6, baojingneirong);//ˢ�±�������
//			
//			sprintf((char*)baojingneirong,"�������� �豸���:%s ��ַ����:%d",str,shield_sum);
//			SetTextValue(52, 7, baojingneirong);//ˢ�±�������
	}
	if(current_screen_id==52)                                              
	{
		uint8_t baojingneirong[50];
		char* str = "������̽����";
		sprintf((char*)baojingneirong,"�豸���� �豸���:%s ��ַ����:%d",str, ds.curr_num);
		SetTextValue(52,2,baojingneirong);//ˢ�±�������
		
		sprintf((char*)baojingneirong,"�������� �豸���:%s ��ַ����:%d",str, ds.curr_num - ( pack_disconnect_sum + cabin_disconnect_sum ));
		SetTextValue(52, 5, baojingneirong);//ˢ�±�������
		
		sprintf((char*)baojingneirong,"�������� �豸���:%s ��ַ����:%d",str, ( pack_disconnect_sum + cabin_disconnect_sum ));
		SetTextValue(52, 6, baojingneirong);//ˢ�±�������
	...
	else if(current_screen_id == 46) 
	{
		...
		if(qingchujilu==1)
		{
			SetTextValue(current_screen_id, 7, "���������¼... ��ȴ���");
			BspClearFlashData();
			SetTextValue(current_screen_id, 7, "�����ɣ�");
			qingchujilu=0;
		}
	}
	...
	else if(current_screen_id == 59)
	{
		...
		// NEW 
		// 2025/12/09 10:22 ����ѭ����ʾ
		if(pcfws.self_bottom_point > Alarm_Show_Zone) // �������������������ʾ���� 
		{
	...
	else if(current_screen_id == 61 || current_screen_id == 62) // ˢ�²�һ��
	{
		// ����PACK��ʾ
//		InternalScreenShowClusterData_32Pack(current_screen_id, &ddsc_32p);
	...
	// 	/* �ؼ�200����·���� */
	// 	sprintf((char *)temp_buff, "��ǰ��ʾ��·��%d ��·", pack_circuit);
	// 	SetTextValue(current_screen_id, 200, temp_buff);
	...
	else if (current_screen_id == 69)
	{
		...
		for (ctrl_idx = 0, list_idx = start; list_idx < end; ctrl_idx++, list_idx++)
		{
			...
			/* XR5000_SCREEN69_FLICKER_FIX_20260727: ����ԭʼ����ֵ�Ƚϣ����strcmp�ַ����Ƚϣ�����Ƶ�� */
	...
	LogicScreen_UpdateUI(current_screen_id); /* ˢ���߼���Ļ��ǰ���� */
	
	FireAlarmThresholdUpdataUI(current_screen_id, fire_alarm_threshold);
	/* �¼ӹ��ܣ�FCP-1011��·���ư壻ʱ�䣺2026-08-06 */
...
*  \param state ��ť״̬��0����1����
*  \param tubiaobh ͼ��֡��ţ�0��0֡��1��1֡��2��2֡...
*/
...
void NotifyButton(uint16 screen_id, uint16 control_id, uint8  state)
{
	...
	if(screen_id == 1)
	{
		...
		else if(control_id == 32 && state == 1)  // ��������ǵ���ʱ��
		{
		...
		else if( (control_id == 31 || control_id == 35) && state == 1 )
		{
			...
			{
				setKeyValue(DEVICE_CTRL_KEY); // ��������ֵ �������޸���Ļ�İ�������
				SwitchCurrentScreenId(53);
				bsp_screen_switch_ctrl.target_screen = 53;
				bsp_screen_switch_ctrl.switch_flag = 1;
			}
	...
	else if(screen_id == 4) // ������µ��豸�����߽���
	{
		if(control_id == 45 && state == 1) // һ�����߰���
		{
		...
		else if(control_id == 55 && state == 1) // һ�����߰���
		{
			uint8_t modify_flag = 0;
			for(uint8_t i = 0; i < 32; i++)
			{
	...
	else if(screen_id == 8)
	{
		if(state == 1)
		{
			...
			{
				setKeyValue(SIMU_SERIAL_PORT); // ��������ֵ �������޸���Ļ�İ�������
			...
			{
				setKeyValue(LINKAGE_PROGREM); // ��������ֵ �������޸���Ļ�İ�������
	...
	else if(screen_id ==17)//�豸�����ߴؼ�ҳ��
  {
			if(state==0)//����
			{
				switch(control_id)
				{
					case 5: cu_sxzt[1]=0;cu_tcq_sxzt[1]=0;CU_zx_buf[1]=0;break;//20240202������CU_zx_buf[1]=0;��ֹ���ص��ߺ��������ߣ���Ĵ��ٱ�������ʾ��Ų���
				...
				SetTextInt32(17,6,cu_tcq_sxzt[1],0,1);//�����ı�Ϊ��������ҳ�棬�ؼ�����ֵ��0-�޷��š�1-�з��ţ�����λ��������ʱ��ಹ�㣩
	...
	else if(screen_id == 19)
  {
	...
	else if(screen_id ==26)//
	{
		...
		if(state==1)//����
		{
			...
			Save_cang_pbzt();//��ִ�����״̬ pack_bianhaobuf
		}
	}
		...
		if(control_id == 4 && state == 1) 
		{
			...
			if(yonghumima == SystemSaveInfo.user_password)                                                       
			{
				switch(getKeyPressValue())
				{
					...
					case SIREN_KEY:    // ����������
						screen_show_siren_information ^= 0x0F; // ��ת����λ״̬
						break;
	...
	LogicScreen_OnButton(screen_id, control_id, state); /* �߼���Ļ��ť���� */
...
{
   { 
			if(control_id == 25) // �޸�CAN2ID��ַ
      {
				int32 value=0;  			
				sscanf((const char*)(char*)str,"%ld",&value); 
				SystemSaveInfo.can2_slave_addr = value;//MODBUS��ַ
				SystemInfoSave();
				SystemInfoLoad();
			}
			else if(control_id == 30) // �޸ĳ�վ485��ַ
      {
				int32 value=0;  			
				sscanf((const char*)(char*)str,"%ld",&value); 
				SystemSaveInfo.slave_addr485_Station = value;//MODBUS��ַ
				SystemInfoSave();
				SystemInfoLoad();
			}
			else if(control_id == 22)
			{
				int32 value=0;  			
				sscanf((const char*)(char*)str,"%ld",&value); 
				SystemSaveInfo.slave_addr485_EMS = value;//MODBUS��ַ
				SystemInfoSave();
				SystemInfoLoad();
			}
		}
		...
		else if(screen_id == 4) // 
		{
			...
			if(temp_str_len != 0)
			{
				if(control_id == 2)
				{
					...
					if(success_len == 2 && x >0 && y > 0) // �����ɹ� ����x y������
					{
						uint8_t modify_flag = 0;
						if (x > y)
						{
    ...
    {                                                                           
        int32 value=0;  
        sscanf((const char*)str, "%ld", &value);    //���ַ���ת��Ϊ���� 
				switch(control_id)
				{
					case 6:if(cu_sxzt[1]==1){cu_tcq_sxzt[1]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 9:if(cu_sxzt[2]==1){cu_tcq_sxzt[2]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 12:if(cu_sxzt[3]==1){cu_tcq_sxzt[3]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 15:if(cu_sxzt[4]==1){cu_tcq_sxzt[4]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 18:if(cu_sxzt[5]==1){cu_tcq_sxzt[5]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 21:if(cu_sxzt[6]==1){cu_tcq_sxzt[6]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 26:if(cu_sxzt[7]==1){cu_tcq_sxzt[7]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 29:if(cu_sxzt[8]==1){cu_tcq_sxzt[8]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 32:if(cu_sxzt[9]==1){cu_tcq_sxzt[9]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 35:if(cu_sxzt[10]==1){cu_tcq_sxzt[10]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 38:if(cu_sxzt[11]==1){cu_tcq_sxzt[11]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 41:if(cu_sxzt[12]==1){cu_tcq_sxzt[12]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					
					case 49:if(cu_sxzt[13]==1){cu_tcq_sxzt[13]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 52:if(cu_sxzt[14]==1){cu_tcq_sxzt[14]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 55:if(cu_sxzt[15]==1){cu_tcq_sxzt[15]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 58:if(cu_sxzt[16]==1){cu_tcq_sxzt[16]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 61:if(cu_sxzt[17]==1){cu_tcq_sxzt[17]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 64:if(cu_sxzt[18]==1){cu_tcq_sxzt[18]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 67:if(cu_sxzt[19]==1){cu_tcq_sxzt[19]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
					case 70:if(cu_sxzt[20]==1){cu_tcq_sxzt[20]=value;kaijiyanshi=0;}break;//��ȡ���������ı���ֵ
				}
				Save_cutcq_sxzt();
				SetTextInt32(17,6,cu_tcq_sxzt[1],0,1);//�����ı�Ϊ��������ҳ�棬�ؼ�����ֵ��0-�޷��š�1-�з��ţ�����λ��������ʱ��ಹ�㣩
				SetTextInt32(17,9,cu_tcq_sxzt[2],0,1);
		...
		else if(screen_id==23)                                                                 //����ID2���ı����ú���ʾ
    {
			if(mimajiyi == 0) // �û�δ�������� ���� 4.25�����ѹ� ��Ҫ������������
			{
				if(control_id==2)                                                            //
				{
					int32 value=0;  			
					sscanf((const char*)(char*)str,"%ld",&value);                                                    //���ַ���ת��Ϊ���� 					
					yonghumima=value;													//����
				}  
			}                                                                     
    }
		else if(screen_id == 24)                                                                 //����ID2���ı����ú���ʾ
    {                                                                            
        int32 value=0;  			
        sscanf((const char*)(char*)str,"%ld",&value);                                                    //���ַ���ת��Ϊ���� 
				if(control_id==2)                                                            //
		...
		else if(screen_id==25)                                                                 //����ID2���ı����ú���ʾ
    {                                                                            
		...
		else if(screen_id == 27)
		{
			if(control_id == 9) // ��������֤����
			{
				...
				if(strncmp((char *)SystemSaveInfo.curr_license_store, (char *)SystemSaveInfo.pref_license_store, 10) == 0 && SystemSaveInfo.license_remain_day == 6666) // �Ƚ�Ԥ��ID �� ��ʼ������
				{
					for(uint8_t i = 0; i < 10; i++)
					{
					...
					SystemSaveInfo.license_remain_day = 30; // Ĭ������30��
					
					uint8_t slicense_buff[32] = {0};
					sprintf((char *)slicense_buff, "����״̬:ʣ��%d��", SystemSaveInfo.license_remain_day);
					SetTextValue(27, 7, slicense_buff);
				}
				else // ������ĺ���һ�εĲ�һ��
				{
					...
					{
						SetTextValue(27, 7, "����״̬:��Ч");
						SystemSaveInfo.license_remain_day = 0; // ʣ����������
					}
					
				}
				SystemInfoSave();
				last_time_stamp -=3600000;
				SetTextValue(27, 9, "����ʹ������֤");
			}
		}
		else if(screen_id == 41) // ʱ���޸Ľ���
		{
			// ��������
...
void NotifyReadRTC(uint8 year,uint8 month,uint8 week,uint8 day,uint8 hour,uint8 minute,uint8 second)
{
    ...
    secs    =(0xff & (second>>4))*10 +(0xf & second);                                    //BCD��תʮ����
...
//	}
	...
	ds_entry->curr_num = detector_sum; // ���豸������ֵ
}
static uint8_t getPointDetectorSetUpCount(void)
{
...
uint8_t creatNewFaultRecordToCache(uint8_t cluster_id, uint8_t pack_id, uint8_t cabin_id)
{
	...
	if(flag != 1) // ���û����ͬ��
	{
		...
		if(cluster_id == LINKAGE_CLUSTER_ID) // �����ID���������豸���
		{
			pcfs[pcfs_buttom_point].detector_class = LinkageClassID; // ��ȷ̽���������������豸
			pcfs[pcfs_buttom_point].da.cabin_id    = cabin_id;       // ���ֺ������䱨������ 
			pcfs[pcfs_buttom_point].da.cluster_id  = cluster_id;
			pcfs[pcfs_buttom_point].da.pack_id     = pack_id;
		}
		else if(cluster_id != 0)
		{
			pcfs[pcfs_buttom_point].detector_class = PackClassID; // ��ȷ̽���������ǰ�
			pcfs[pcfs_buttom_point].da.cabin_id    = 0;           // ��ղ�ID��� ����ǰ���Ĭ�����
			pcfs[pcfs_buttom_point].da.cluster_id  = cluster_id;
			pcfs[pcfs_buttom_point].da.pack_id     = pack_id;
		}
		else
		{
			pcfs[pcfs_buttom_point].detector_class = CabinClassID; // ��ȷ̽���������ǲ�
...
void deletRecoveryRecord(uint8_t recovery_index)
{
	...
	{
		pcfs[k].detector_class = pcfs[k + 1].detector_class; // ����һ������ǰһ��
...
static uint8_t ClusterPackDataDeal(PackCabinFaultStorage *pcfs_entry, uint8_t *pcfs_point)
{
	...
	{
		if(cu_tcq_sxzt[jsz] == 0) // ���û��������Ҳ��û�д����ı�Ҫ��
		{
			continue;
		}
		for(uint8_t i=1;i<=cu_tcq_sxzt[jsz];i++)//ѭ������������������������ ����ÿһ���µİ�
		{
			if(getClusterPackDisconnectCount(jsz, i) == PackDisconnectCount) { // �������Ϊ���� �ж��Ƿ����
				// �������� �����й�����Ϣͳһ����
	
				disconnect_detector_sum++; // �����豸+1
				// һ��Ҫд0��������
				if(creatNewFaultRecordToCache(jsz, i, 0) == 0) // �������0��ʾ�ɹ�д�� ��Ҫ���������� ���洢FLASH
				{
					beep_fault_ctrl  = 2;   // �������� ���Ϸ�������־λ
					silencers_state  = 0;   // ��������
					disconnect_state = 1;   // �������ϵ�
					// �޸�Ϊ�����洢
					//DebugSendString((uint8_t *)&temp_data, sizeof(FlashSaveDetectFault_t));
					BspCommonDataSaveApp(FAULT_FLASH_SAVE, DISCONNECT, jsz, i);
				}
			}else if(PACK_zx_buf[jsz][i] == i) { //�������ֵ�Ƕ�Ӧ��ID��� ���� �ڽ����ж�
				// new
				...
				else if(BJ_packjiyibuf_co[jsz][i] != 0 && PACK_COZT_buf[jsz][i] == 0) { // ֤��֮ǰ�洢��
					// һ����̼�����Իָ�����Ҫ��������ɾ��
					// NEW
					// 2025/10/11 10:35 ��ȼ�����Իָ�ָ����̽������������ ���Բ���Ҫ�Իָ�
...
//					BspAlarmDataSaveApp(FIRE_FLASH_SAVE, EAR_RECOVERY, jsz, i, getPackCoConcenValue(jsz, i));
			...
			if((( BJ_packjiyibuf_ch4[jsz][i] != 0 || 
						...
						BJ_packjiyibuf_yw[jsz][i] != 0) && 
						BJ_packjiyibuf_wd[jsz][i] != 0) 
				//		|| BJ_packjiyibuf_wd[jsz][i] == 2 // ��ʱ���ε������¶ȱ���������� ���������
				)
			{
				// ���ϻ��ж�
				uint8_t flag = 0;
				...
				{
					if(pas[j].cabin_id == 0) // �����ID����0 ������λ�ô洢�Ĳ��ǲ�
					{
				...
				if(flag != 1) // ��ʾû�д洢��
				{
					...
					pas[pas_pointer].lunch_state = 0; // ����״̬ δ����	
					// ʱ�丳ֵ
	...
	if(disconnect_detector_sum < *pcfs_point) // ��������豸��С��ָ������ ֤�����豸�ָ���
	{
		uint8_t flag = 0; 
		uint8_t k;
...
static uint8_t ClusterPackDataDeal_Plus(PackCabinFaultStorage *pcfs_entry, uint8_t *pcfs_point)
{
	...
	{
		for(uint8_t i = 1; i < 33; i++)//ѭ������������������������ ����ÿһ���µİ�
		{
			if(pack_online_buff[jsz][i] == 0) // ���̽����δ����
			{
				continue;
			}
			if(getClusterPackDisconnectCount(jsz, i) == PackDisconnectCount) // �������Ϊ���� �ж��Ƿ����
...
static uint8_t CabinDataDeal(PackCabinFaultStorage *pcfs_entry, uint8_t *pcfs_point)
{
	...
	//ѭ���ж� 24 ���� ̽������ ����״̬
	for(uint8_t jsz=1; jsz <= 24; jsz++) // �����������ĸ���̽������ѭ������ҲҪ�ĳ�24
	{
		if(cang_sxzt[jsz] != 1) // ���û���������� ֱ���ж���һ��
		{
			continue;
		}
		else if(Cang_zx_buf[jsz] == CabinDisconnectCount) // ������߼������ ������+1 �ж���һ��
		{
			temp_cabin_disconnect_sum++; // ����ֵ�������	

			if(DX_cangjiyibuf[jsz] == 0)
			{
				DX_cangjiyibuf[jsz] = 1;	
				if( creatNewFaultRecordToCache(0, 0, jsz) == 0 ) // ���д��ɹ�
				{
					beep_fault_ctrl  = 2;   // �������� ���Ϸ�������־λ
					silencers_state  = 0;   // ��������
					disconnect_state = 1;   // �������ϵ�
					// �޸�Ϊ�����洢
					//DebugSendString((uint8_t *)&temp_data, sizeof(FlashSaveDetectFault_t));
					BspCommonDataSaveApp(FAULT_FLASH_SAVE, DISCONNECT, 0, jsz); // �洢�ֵ���
				}
			}
			continue;
		}
		...
		if(Cang_WDZT_buf[jsz] != 0 && BJ_cangjiyibuf_wd[jsz] == 0) 
		{
			...
			// ��������
			cabin_detector_state_buff[jsz].temperature_state = 1; // �¶�Ԥ��
			// end
		}
		...
		if(Cang_YWZT_buf[jsz] != 0 && BJ_cangjiyibuf_yw[jsz] == 0) { 
			...
			// ��������
			cabin_detector_state_buff[jsz].smoke_state = 1; // ����һ��Ԥ��
			// end
		}
		...
		{ 
			...
			// ��������
			cabin_detector_state_buff[jsz].carbon_state = Cang_COZT_buf[jsz]; // һ����̼һ��Ԥ��
		...
		else if(Cang_COZT_buf[jsz] == 0 && BJ_cangjiyibuf_co[jsz] != 0)
		{
			...
			// ��������
			cabin_detector_state_buff[jsz].carbon_state = 0;      // �������״̬
		}
			
//			if(Cang_CH4ZT_buf[jsz]==1 && cang_pbzt[jsz]==0)//�ּ���һ��Ԥ��
//			{
...
//				}
//			}else if(Cang_CH4ZT_buf[jsz]==2 && cang_pbzt[jsz]==0) { //�ּ������Ԥ��
...
//			}
		...
		{ 
			...
			// ��������
			cabin_detector_state_buff[jsz].hydrogen_state = 1; // ����һ��Ԥ��
		...
		else if(Cang_H2ZT_buf[jsz] == 0 && BJ_cangjiyibuf_h2[jsz] != 0) 
		{
			...
			// ��������
			cabin_detector_state_buff[jsz].hydrogen_state = 0;    // �������״̬
			BJ_cangjiyibuf_h2[jsz] = 0;
		}

		// end

		// 2025/10/27 16:42 ������ȼ����Ũ���ж�
			...
			&& BJ_cangjiyibuf_wd[jsz] != 0 ) )
		{
			...
			{
				if(pas[k].cluster_id == 0 && pas[k].pack_id == 0) // �����ID�Ͱ�ID����0 ������λ�ô�����ǲ�ID
				{
					if(pas[k].cabin_id == jsz) // �����ID�Ѿ��洢��
					{
						flag = 1;
						break; // ����ñ�������Ѿ��洢���� ����ѭ��
					}
	...
	return temp_cabin_disconnect_sum; // ���زֵ�������
}

static void FaultRelayCtrlAppFun(uint8_t disconnect_num)
{
	if(disconnect_num != 0) // �������������Ϊ0 ���Ϲ��ϸɽӵ�
	{
...
static void InternalScreenShowAllFault(uint8_t fresh_page_flag)
{
	...
	if(total_fault_count == 0U)
	{
		...
		{
			disconnect_state = 0;  // ����״̬��� 
			beep_fault_ctrl  = 0;  // �رյ��߷�����
			pcfs_fresh_ctrl = 0;
			last_product_unknown_count = 0U;
			clearTextValue(monitor_inform_screen_id , 43);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 44);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 45);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 46);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 47);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 48);//(����ID,�ؼ�ID)
		}
		switch(screen_fresh_num)
		{
			case 10:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������.     ");break;   //ˢ�±�������
			case 20:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������..    ");break;   //ˢ�±�������
			case 30:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������...   ");break;   //ˢ�±�������
			case 40:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������....  ");break;   //ˢ�±�������
			case 50:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������..... ");break;   //ˢ�±�������
			case 60:SetTextValue(monitor_inform_screen_id, 42,"���ϼ��������......");break;   //ˢ�±�������
		}
	}
	else if(pcfs_fresh_ctrl != pcfs_buttom_point || last_product_unknown_count != product_unknown_count || fresh_page_flag == 1)
	{
		...
		for (uint8_t i = 0; i < Fault_Show_Zone; i++) {
			...
			if(data_index < total_fault_count) {
				...
				else if(pcfs[data_index].detector_class == PackClassID) // ��������ǰ�
				{
					// 2025/11/19 10:59 ������¼������
				...
				else if(pcfs[data_index].detector_class == CabinClassID) // ��������ǲ�
				{
					sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� ����", temp_sequence_count, // ������ʾ���
						pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
						pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second,
				...
				else if(pcfs[data_index].detector_class == LinkageClassID) // ����������豸
				{
					switch(pcfs[data_index].da.pack_id)
					{
						...
						case SoundLt_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���ⱨ��������", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
						...
						case SirenBk_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���ѵ���", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���Ѷ�·", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							break;
						case OutFir1_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���װ��1����", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
						...
						case OutFir2_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���װ��2����", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
						...
						case CabinBK_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���װ�õ���", temp_sequence_count,
								pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
								pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ���װ�ö�·", temp_sequence_count,
								pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
								pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
						...
						case HANDPOT_Package_ID:
							if(pcfs[data_index].da.cabin_id == DISCONNECT)
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d �ֱ�����", temp_sequence_count,
									pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
									pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
							}
							else
							...
							break;
						case SYS_FLASH_FAULT_ID: { // ����Ǵ洢����
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ϵͳ�洢����", temp_sequence_count,
								pcfs[data_index].atr.years, pcfs[data_index].atr.months, pcfs[data_index].atr.days,
								pcfs[data_index].atr.hours, pcfs[data_index].atr.minute, pcfs[data_index].atr.second);
				...
				baojingneirong[0] = 0;
				//clearTextValue(1 , 41 + i);//(����ID,�ؼ�ID��
			}
			SetTextValue(monitor_inform_screen_id, i + 42, baojingneirong);//ˢ�±�������
		}
	}
...
{
	// Ԥ����ʾ
	if(pcfws_entry->self_bottom_point == 0) // �����������Ϊ0 
	{
		if(pcfws_entry->point_history_len != 0)
		{
			pcfws_entry->point_history_len = 0;
			clearTextValue(monitor_inform_screen_id , 36);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 37);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 38);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 39);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 40);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 41);//(����ID,�ؼ�ID)
		}
		switch(screen_fresh_num)
		{
			case 10:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������.     ");break;   //ˢ�±�������
			case 20:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������..    ");break;   //ˢ�±�������
			case 30:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������...   ");break;   //ˢ�±�������
			case 40:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������....  ");break;   //ˢ�±�������
			case 50:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������..... ");break;   //ˢ�±�������
			case 60:SetTextValue(monitor_inform_screen_id, 35,"Ԥ��ϵͳ������......");break;   //ˢ�±�������
		}
	}
	else if(pcfws_entry->self_bottom_point != pcfws_entry->point_history_len || fresh_page_flag == 1)
	{
		...
		else if(pcfws_entry->detector_class[0] == PackClassID)
		{
			...
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽�����¶ȱ���", 1,
			...
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽������������", 1,
			...
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽����һ����̼����", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cluster_id, pcfws_entry->da[0].pack_id);
			}
		...
		else if(pcfws_entry->detector_class[0] == CabinClassID)
		{
			if(pcfws_entry->alarm_type[0] == Temperature)
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �¶ȱ���", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
			...
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� һ����̼����", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cabin_id);
			}
		...
		SetTextValue(monitor_inform_screen_id, 35, baojingneirong); // ˢ�µ�һ����������
			
		uint8_t temp_sequence_count = 0;
		
		// ʣ�µ����������ʾ
		for (uint8_t i = 1; i < Alarm_Show_Zone; i++) {
			uint8_t data_index = fore_alarm_start_index + i; // Ԥ������
			...
			else if(data_index < pcfws_entry->self_bottom_point && pcfws_entry->detector_class[data_index] == PackClassID)
			{
				...
				{
					sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽�����¶ȱ���", temp_sequence_count,
						pcfws_entry->atr[data_index].years, pcfws_entry->atr[data_index].months, pcfws_entry->atr[data_index].days,
						pcfws_entry->atr[data_index].hours, pcfws_entry->atr[data_index].minute, pcfws_entry->atr[data_index].second,
				...
				{
					sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽������������", temp_sequence_count,
						pcfws_entry->atr[data_index].years, pcfws_entry->atr[data_index].months, pcfws_entry->atr[data_index].days,
						pcfws_entry->atr[data_index].hours, pcfws_entry->atr[data_index].minute, pcfws_entry->atr[data_index].second,
						pcfws_entry->da[data_index].cluster_id, pcfws_entry->da[data_index].pack_id);
				}
				...
				{
					sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽����һ����̼����", temp_sequence_count,
						pcfws_entry->atr[data_index].years, pcfws_entry->atr[data_index].months, pcfws_entry->atr[data_index].days,
						pcfws_entry->atr[data_index].hours, pcfws_entry->atr[data_index].minute, pcfws_entry->atr[data_index].second,
						pcfws_entry->da[data_index].cluster_id, pcfws_entry->da[data_index].pack_id);
				}
			...
			{
				baojingneirong[0] = 0;
//				clearTextValue(1 , 35 + i); //(����ID,�ؼ�ID��
			}
			SetTextValue(monitor_inform_screen_id, 35 + i, baojingneirong); // ˢ�±�������
		}
	}
}
...
static void InternalScreenShowAllFireAlarm(PackCabinFireAlarmStorage *pcfas_entry, uint8_t fresh_page_flag)
{
	...
	if(pcfas_entry->self_bottom_point == 0) // 
	{
		if(pcfas_entry->point_history_len != 0)
		{
			...
			clearTextValue(monitor_inform_screen_id , 50);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 51);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 52);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 53);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 54);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 55);//(����ID,�ؼ�ID)
		}
		switch(screen_fresh_num)
		{
			case 10:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������.     ");break;   //ˢ�±�������
			case 20:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������..    ");break;   //ˢ�±�������
			case 30:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������...   ");break;   //ˢ�±�������
			case 40:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������....  ");break;   //ˢ�±�������
			case 50:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������..... ");break;   //ˢ�±�������
			case 60:SetTextValue(monitor_inform_screen_id, 49,"��ϵͳ������......");break;   //ˢ�±�������
		}
	}
	else if(pcfas_entry->self_bottom_point != pcfas_entry->point_history_len || fresh_page_flag == 1)
	{
		...
		else if(pcfas_entry->detector_class[0] == LinkageClassID)
		{
			...
			{
				sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d �ֱ����±���", 1,
					pcfas_entry->atr[0].years, pcfas_entry->atr[0].months, pcfas_entry->atr[0].days,
					pcfas_entry->atr[0].hours, pcfas_entry->atr[0].minute, pcfas_entry->atr[0].second);
			}
		...
		SetTextValue(monitor_inform_screen_id, 49, baojingneirong); // ˢ�µ�һ����������
		...
		for (uint8_t i = 1; i < Alarm_Show_Zone; i++) {
			...
			{
				baojingneirong[0] = 0;
//				clearTextValue(1 , 38 + i); //(����ID,�ؼ�ID��
			}
			SetTextValue(monitor_inform_screen_id, 49 + i, baojingneirong); // ˢ�±�������
		}
	}
}
...
)
{
	...
	fedas_entry->countdown_val[fedas_entry->self_point_len] = state_switch_delay; // ����ʱ��state_switch_delay��
	// ��¼����ʱ��
...
static void FireExtinguishDevice1HandStart(FireExtinguishDeviceActionSave *fedas_entry)
{
	...
	for(uint8_t j = 0; j < fedas_entry->self_point_len; j++)
	{
		if(fedas_entry->cabin_id[j] == 0 && fedas_entry->cluster_id[j] != OUTFIRE_CLUSTER_ID && fedas_entry->cluster_id[j] != 0)
		{
			...
			{
				// ��״̬ ��ʾ���ݲ��� ������ʾ���װ������ֹͣ
				fedas_entry->fed_action[j] = FIRE_EXTINGUISH_RESTART_FINISH; // ���Ϊ�Ѿ���������
				// �����¼�¼ ��������
			...
			{
				fedas_entry->start_cntd_time[j] = baojingjishi; // ��¼����ʱ��
				fedas_entry->curr_cntd_time[j]  = baojingjishi; // ��¼һ�µ�ǰʱ��
	...
	if(out_fire_start_flag == 0) // ������ΰ���û�������κδ�
	{
		...
		// ����һ��������¼
		fedas_entry->start_cntd_time[index] = baojingjishi; // ��¼����ʱ��
		fedas_entry->curr_cntd_time[index]  = baojingjishi; // ��¼һ�µ�ǰʱ��
		fedas_entry->fed_action[index]      = FIRE_EXTINGUISH_START_SPRAY_DELAY; 
		
		fedas_entry->countdown_val[index] = 30; // ����ʱ��state_switch_delay��
...
static void FireExtinguishDevice2HandStart(FireExtinguishDeviceActionSave *fedas_entry)
{
	...
	if(out_fire_start_flag == 0) // ������ΰ���û���κβ����� ���ֶ�ǿ������
	{
		uint8_t index = fedas_entry->self_point_len;
		...
		fedas_entry->countdown_val[fedas_entry->self_point_len] = 30; // ����ʱ��state_switch_delay��
...
{
	// �����̽������
	if(pas_pointer != last_pas_len && pas_pointer > 0) // ������µı�������
	{
		for(uint8_t i = last_pas_len; i < pas_pointer; i++) // ����һ�μ�¼��λ�ÿ�ʼ���ṹ����и�ֵ
		{
			...
			fedas_entry->fed_action[fedas_entry->self_point_len] = pas_entry[i].lunch_state; // ��ȡ��ǰ����״̬
			fedas_entry->countdown_val[fedas_entry->self_point_len] = 30; // ������ʱ����ʱ30��
	...
	if(getFeedBack1State() == 0x0F)
	{
		...
		fedas_entry->fed_action[fedas_entry->self_point_len] = FEEDBACK_1_PRESS; // ��ȡ��ǰ����״̬
	...
	if(getOutFireKeyalue() == KEY12_PART1_STOP)
	{
		...
		for(uint8_t j = 0; j < fedas_entry->self_point_len; j++)
		{
			if(fedas_entry->cabin_id[j] == 0  && fedas_entry->cluster_id[j] != OUTFIRE_CLUSTER_ID && fedas_entry->cluster_id[j] != 0)
			{
				...
				{
					// ���״̬��ʾΪ ʱ�� ��������ʱ������
					fedas_entry->fed_action[j] = FIRE_EXTINGUISH_FORCE_STOP; // ���Ϊǿ�ƽ���
					// ����״̬��ֵΪ���������� ��ʾ����Ļ���� ֹͣ����
	...
	for(uint8_t i = 0; i < fedas_entry->self_point_len; i++) // �������б����ĵ���ʱ 
	{
		if(fedas_entry->cluster_id[i] == LINKAGE_CLUSTER_ID) // �����ID���������豸������������ѭ��
		{
			continue;
		}
		fedas_entry->curr_cntd_time[i] = baojingjishi; // ��ȡһ�µ�ǰʱ��
		switch(fedas_entry->fed_action[i])
		{
			case FIRE_EXTINGUISH_MODE_JUDGEMENT: { // �ж����ֶ������Զ�
				if(fedas_entry->cluster_id[i] == 0)
				{
					if(getPart2HandAutoState() == KEY_AUTO)
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��¼����ʱ��
						fedas_entry->curr_cntd_time[i]  = baojingjishi; // ��¼һ�µ�ǰʱ��
						fedas_entry->fed_action[i]      = FIRE_EXTINGUISH_START_SPRAY_DELAY; // �Զ���������ʱ
			...
			case FIRE_EXTINGUISH_START_SPRAY_DELAY: { // ���װ�õ�1����������ʱ
				// ��BMS�ϴ���״̬
				...
				if(fedas_entry->cluster_id[i] == 0)
				{
					...
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FED_START_SPRAY_DELAY_FINISH_FLAG; // ���Ϊ��һ״̬���
						// �ر�������ʱLED
						...
						DefauleRelayCtrl(JDQ_ON);
						// ������ƿ������¼
						CreatNewFireExtinguishRecord(fedas_entry , fedas_entry, i, FIRE_EXTINGUISH_CYLINDEF_2_OPENED, 0);  // ��ƿ1��ŷ��� 
						// ������ʼ��ż�¼
				...
				else
				{
					...
					if(fedas_entry->curr_cntd_time[i] - fedas_entry->start_cntd_time[i] >= fedas_entry->countdown_val[i])
					{
						...
						DefauleRelayCtrl(JDQ_ON);
						// ������ƿ������¼
						CreatNewFireExtinguishRecord(fedas_entry , fedas_entry, i, FIRE_EXTINGUISH_CYLINDEF_1_OPENED, 0);  // ��ƿ1��ŷ��� 
						// ������ʼ��ż�¼
						CreatNewFireExtinguishRecord(fedas_entry , fedas_entry, i, FIRE_EXTINGUISH_FIRST_SPRAY_START, 15); // �������15��
						// �Ѿ��������
						outfire_spray_state = 2; 
			...
			case FIRE_EXTINGUISH_FIRST_SPRAY_START: { // ���װ����ų���ʱ��
				if(fedas_entry->cluster_id[i] == 0)
				{
					Part2StartDelayLedCtrl(LED_ON);
					// �����ų���ʱ�乻�� ����״̬�����¼
					...
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FIRE_EXTINGUISH_FIRST_SPRAY_FINISH; // ��һ�γ�����Ž���
						
						// ��ŵ���ʱ������ �رո�ƿ��ŷ�
						OutFire1RelayCtrl(JDQ_OFF);
						
						// 2025/10/28 10:50 ��ʱ��ֹͣ���ʱ���رշ����������
//						// �رշ����������
//						DefauleRelayCtrl(JDQ_OFF);
...
//						DefauleRelayCtrl(JDQ_OFF);
			...
			case FIRE_EXTINGUISH_SECOND_SPRAY_DELAY: {   // ���װ�õ�2����������ʱ
				...
				if(fedas_entry->cluster_id[i] == 0)
				{
					...
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FED_SECOND_SPRAY_DELAY_FINISH_FLAG; // �ڶ�����ŵ���ʱ����
						// ��������ʱ������ �򿪸�ƿ��
						OutFire2RelayCtrl(JDQ_ON);
						// �򿪷�������
						DefauleRelayCtrl(JDQ_ON);
			...
			case FIRE_EXTINGUISH_SECOND_SPRAY_START: { // ���װ�õڶ��γ������ 
				if(fedas_entry->cluster_id[i] == 0)
				{
					// ��ʱ�ڼ������ʱ
					Part2StartDelayLedCtrl(LED_ON);
					...
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FIRE_EXTINGUISH_SECOND_SPRAY_FINISH; // �ڶ�����Ž���
						// 
						Part2StartDelayLedCtrl(LED_OFF);
						// ��ŵ���ʱ������ �رո�ƿ��ŷ�
						OutFire2RelayCtrl(JDQ_OFF);
...
//						DefauleRelayCtrl(JDQ_OFF);
			...
			case FIRE_EXTINGUISH_THIRD_SPRAY_DELAY: {   // ���װ�õ�3����������ʱ
				if(fedas_entry->cluster_id[i] == 0)
				{
					// ��ʱ�ڼ������ʱ
					Part2StartDelayLedCtrl(LED_ON);
					...
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FED_THIRD_SPRAY_DELAY_FINISH_FLAG; // �ڶ�����Ž���
						// ������ر���ʱLED
						Part2StartDelayLedCtrl(LED_OFF);
						// ��������ʱ������ �򿪸�ƿ��
						OutFire2RelayCtrl(JDQ_ON);
			...
			case FIRE_EXTINGUISH_THIRD_SPRAY_START: {  // ��������ſ�ʼ
				if(fedas_entry->cluster_id[i] == 0)
				{
					if(fedas_entry->curr_cntd_time[i] - fedas_entry->start_cntd_time[i] >= fedas_entry->countdown_val[i])
					{
						fedas_entry->start_cntd_time[i] = baojingjishi; // ��������ʱ��
						fedas_entry->fed_action[i]      = FIRE_EXTINGUISH_THIRD_SPRAY_FINISH; // ��������Ž���
...
//						DefauleRelayCtrl(JDQ_OFF);
			...
			case FIRE_EXTINGUISH_FORCE_STOP : { // �����ǿ��ֹͣ״̬ ��������
				
				
				break;
			}
			case FIRE_EXTINGUISH_CAN_RESTART: { // ����ǿ�������״̬ �жϰ������º���������
				break;
			}
			default:
				break;
...
static void InternalScreenShowFireExtinguisher(FireExtinguishDeviceActionSave *fedas_entry, uint8_t fresh_page_flag)
{
	if(fedas_entry->self_point_len == 0) // û�л�
	{
		if(fedas_entry->last_point_len == 255)
		{
			clearTextValue(monitor_inform_screen_id , 2);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 3);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 4);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 5);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 6);//(����ID,�ؼ�ID)
			clearTextValue(monitor_inform_screen_id , 7);//(����ID,�ؼ�ID)
			fedas_entry->last_point_len = 0;
		}
		switch(screen_fresh_num)
		{
			case 10:SetTextValue(monitor_inform_screen_id, 1,"������������.     ");break;   //ˢ�±�������
			case 20:SetTextValue(monitor_inform_screen_id, 1,"������������..    ");break;   //ˢ�±�������
			case 30:SetTextValue(monitor_inform_screen_id, 1,"������������...   ");break;   //ˢ�±�������
			case 40:SetTextValue(monitor_inform_screen_id, 1,"������������....  ");break;   //ˢ�±�������
			case 50:SetTextValue(monitor_inform_screen_id, 1,"������������..... ");break;   //ˢ�±�������
			case 60:SetTextValue(monitor_inform_screen_id, 1,"������������......");break;   //ˢ�±�������
		}
	}
	else if( fresh_page_flag == 1 ) // ����л𾯣�����������¶Ȼ��¶ȳ�������Ԥ����������
	{
		...
		for (uint8_t i = 0; i < Out_Fire_Show_Zone; i++) {
			...
			if(data_index < fedas_entry->self_point_len) // ״̬���ڲ��ж��ǰ����ǲ� �����ⲿ�ж���������
			{
				...
				switch(fedas_entry->fed_action[data_index])
				{
					case FIRE_EXTINGUISH_MODE_JUDGEMENT:
						...
						{
							if(getPart2HandAutoState() == KEY_MANUAL) // �ֶ�
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���ֶ��������װ��", temp_sequence_count,
									fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
									fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second, 
						...
						{
							if(getPart1HandAutoState() == KEY_MANUAL) // �ֶ�
							{
								sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���ֶ��������װ��", temp_sequence_count,
									fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
									fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
					...
					case FIRE_EXTINGUISH_START_SPRAY_DELAY:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
						...
						break;
					case FED_START_SPRAY_DELAY_FINISH_FLAG: // ���װ�õ�һ����������ʱ����
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�õ�1���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second, 
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�õ�1���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_FIRST_SPRAY_START:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�����ʣ��ʱ��%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second, 
								fedas_entry->cabin_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�����ʣ��ʱ��%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]  , fedas_entry->countdown_val[data_index] - temp_time);
						}
					...
					case FIRE_EXTINGUISH_SECOND_SPRAY_DELAY:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�õ�2����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�õ�2����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]  , fedas_entry->countdown_val[data_index] - temp_time);
						}
					...
					case FED_SECOND_SPRAY_DELAY_FINISH_FLAG:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�õ�2���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�õ�2���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_THIRD_SPRAY_DELAY:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�õ�3����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index], fedas_entry->countdown_val[data_index] - temp_time);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�õ�3����������ʱ%d", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]  , fedas_entry->countdown_val[data_index] - temp_time);
						}
						break;
					case FED_THIRD_SPRAY_DELAY_FINISH_FLAG: // ��������ŵ���ʱ����
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ�õ�3���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ�õ�3���������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_ALL_SPRAY_COMPLETE:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ��������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ��������", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]  );
						}
					...
					case FIRE_EXTINGUISH_CYLINDEF_1_OPENED:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ��1����", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ��1����", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_FORCE_STOP: {
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ����������ʱ--", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ����������ʱ--", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_RESTART_FINISH:
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ���ֶ�ֹͣ����", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
						...
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��pack%d�� ���װ���ֶ�ֹͣ����", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second, 
								fedas_entry->cluster_id[data_index], fedas_entry->pack_id[data_index]);
						}
					...
					case FIRE_EXTINGUISH_CYLINDEF_2_OPENED: {
						if(fedas_entry->cluster_id[data_index] == 0)
						{
							sprintf((char*)baojingneirong, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �� ���װ��2����", temp_sequence_count,
								fedas_entry->atr[data_index].years, fedas_entry->atr[data_index].months, fedas_entry->atr[data_index].days,
								fedas_entry->atr[data_index].hours, fedas_entry->atr[data_index].minute, fedas_entry->atr[data_index].second,
								fedas_entry->cabin_id[data_index]);
						}
...
{
	ddsc_entry->curr_detector_page = 1; // Ĭ���ǵ�һҳ
	ddsc_entry->last_detector_page = 0; // Ĭ�ϲ���ʾ
...
{
	ddsc_32p_entry->curr_detector_page = 1; // Ĭ���ǵ�һҳ

	ddsc_32p_entry->last_detector_page = 0xFF; // ��ʼ����ֵ
	...
	ddsc_32p_entry->last_temperature = 0xFF; // ��ʷ�¶�
	ddsc_32p_entry->last_temperat_state = 0xFF; // ��ʷ�¶�״̬
	ddsc_32p_entry->last_smoke_state = 0xFF; // ��ʷ����״̬
	ddsc_32p_entry->last_co_concentrat = 0xFF; // ��ʷһ����̼Ũ��
	ddsc_32p_entry->last_co_state = 0xFF; // ��ʷһ����̼״̬
...
static void InternalScreenShowClusterData(DetectorDataShowCtrl *ddsc_entry)
{
	...
	uint8_t curr_page = ddsc_entry->curr_detector_page; // �ȱ���һ��ָ��
	uint8_t fresh_flag = 0;
	
	if(curr_page !=	ddsc_entry->last_detector_page) // ˢ�µ�ǰҳ��ʾ
	{
		uint8_t buff[32] = {0};
		...
		sprintf((char *)buff, "��%d�� �ؼ�������", curr_page);
	...
	if(cu_sxzt[curr_page] == 0 || cu_tcq_sxzt[curr_page] == 0) // ���û��������
	{
		fresh_flag |= 1; // 
		if(ddsc_entry->detector_offline_fresh_flag[curr_page] != cu_tcq_sxzt[curr_page] || fresh_flag) // ����ǵ�һ��������ҳ��ˢ��
		{
			ddsc_entry->detector_offline_fresh_flag[curr_page] = cu_tcq_sxzt[curr_page]; // �����Ѿ�ˢ�¹���
...
//			fresh_flag = 1;
		//ddsc_entry->detector_offline_fresh_flag[curr_page] = 0xFF; // �������ߺ���Ҫ����ˢ��
		for(uint8_t i = 1; i < cu_tcq_sxzt[curr_page] + 1; i++)
		{
			if(getClusterPackDisconnectCount(curr_page, i) != PackDisconnectCount) // �ж��Ƿ����
			{ 
				// ����״̬ˢ��
				if(ddsc_entry->detect_online_state[curr_page][i] != 1 || fresh_flag)
				{
					fresh_flag |= 2; // �ӵ��߻ָ��� ����ˢ��һ��״̬
					ddsc_entry->detect_online_state[curr_page][i] = 1; // �ѵ�curr_page�� ��i����ֵΪ1
				...
				if(ddsc_entry->last_temperature[curr_page][i] != PACK_wendu_buf[curr_page][i] || fresh_flag)
				{
					...
					SetTextInt32(temp_screen_id, temperature_ctrl_id[i], PACK_wendu_buf[curr_page][i], 1, 2);//�¶���ʾ
				}
				...
				if(ddsc_entry->last_temperat_state[curr_page][i] != PACK_WDZT_buf[curr_page][i] || fresh_flag)
				{
					...
					{
						SetControlForeColor(temp_screen_id, temperature_ctrl_id[i], 0xFB20); // Ԥ��
					}
					else
					{
				...
				{
					ddsc_entry->last_smoke_state[curr_page][i] = temp_co_concen; // �洢һ����̼��ֵ
//					SetTextInt32(temp_screen_id, co_concentrate_ctrl_id[i], temp_co_concen, 1, 1); // CO��ʾ
					uint8_t buff[16];
				...
				if(ddsc_entry->last_co_state[curr_page][i] != PACK_COZT_buf[curr_page][i] || fresh_flag)
				{
					...
					{
						SetControlForeColor(temp_screen_id, co_concentrate_ctrl_id[i], 0xFB20); // Ԥ��
					}
					else if(ddsc_entry->last_temperat_state[curr_page][i] == 2)
					{
						SetControlForeColor(temp_screen_id, co_concentrate_ctrl_id[i], 0xF800); // ����Ԥ��
					}
				}
				...
				if(ddsc_entry->last_smoke_state[curr_page][i] != PACK_YWZT_buf[curr_page][i] || fresh_flag)
				{
					...
					{
						SetControlForeColor(temp_screen_id, smoke_show_ctrl_id[i], 0xFB20); // Ԥ��
						SetTextValue(temp_screen_id, smoke_show_ctrl_id[i], "����");
					}
					else if(PACK_YWZT_buf[curr_page][i] == 2)
					{
						SetControlForeColor(temp_screen_id, smoke_show_ctrl_id[i], 0xF800); // ����Ԥ��
						SetTextValue(temp_screen_id, smoke_show_ctrl_id[i], "����");
					}
...
static void InternalScreenShowClusterData_32Pack(uint16_t screen_id_entry, DetectorDataShowCtrl_32Pack *ddsc_32p_entry)
{
	uint8_t curr_page = ddsc_32p_entry->curr_detector_page; // �ȱ���һ��ָ��
...
//	DebugPrintf("flag:%d\r\n", ddsc_32p_entry->force_fresh_flag);
	...
	// ǿ��ˢ�±�־λ
	if(ddsc_32p_entry->last_detector_page != curr_page) // ˢ�µ�ǰҳ��ʾ
	{
		uint8_t buff[32] = {0};
		sprintf((char *)buff, "��%d�� PACK������", curr_page);
		SetTextValue(screen_id_entry, 1, buff); // ˢ�´�������
		sprintf((char *)buff, "%d/3", curr_page);
		SetTextValue(screen_id_entry, 105, buff); // ˢ�µ�ǰҳ����ʾ
		
		page_change_flage = 1;
		ddsc_32p_entry->last_detector_page = curr_page;
	}

	if(screen_id_entry == 61) // �������̽����������ֵ�鿴����
	{
		// �������״̬Ϊ0 ������������Ϊ0
		if(cu_sxzt[curr_page] == 0 || cu_tcq_sxzt[curr_page] == 0) // ���û��������
		{
			if(ddsc_32p_entry->lat_detector_online_num[curr_page] != cu_tcq_sxzt[curr_page] || page_change_flage == 1) // ����ǵ�һ��������ҳ��ˢ��
			{
				ddsc_32p_entry->lat_detector_online_num[curr_page] = cu_tcq_sxzt[curr_page]; // �����Ѿ�ˢ�¹���
...
//					SetControlForeColor(screen_id_entry, detector_online_ctrl_id[i], 0x8410);
					...
					ddsc_32p_entry->last_derector_state[curr_page - 1][i - 1] = 2; // ���Ϊδ����
				}
...
//						SetControlForeColor(screen_id_entry, detector_online_ctrl_id[i], 0xFB20);
						...
						ddsc_32p_entry->last_derector_state[curr_page - 1][i - 1] = 0; // ���Ϊ�ָ�
					}
					else
					{
//						SetControlForeColor(screen_id_entry, pack_state_show_ctrl_id[i], 0xFB20);
						...
						ddsc_32p_entry->last_derector_state[curr_page - 1][i - 1] = 1; // ���Ϊ����
					}
				}
			...
			else
			{
				...
				for(uint8_t i = 0; i < cu_tcq_sxzt[curr_page]; i++)
				{
					...
					{
						ddsc_32p_entry->last_derector_state[temp_index][i] = 1; // ���Ϊ����
					...
					{
						ddsc_32p_entry->last_derector_state[temp_index][i] = 0; // ���Ϊ�ָ�
						SetTextValue(screen_id_entry, pack_state_show_ctrl_id[i + 1], "����");
					}
	...
	else if(screen_id_entry == 62) // �鿴����̽������ֵ����
	{
		...
		if(cu_sxzt[curr_page] != 0 && cu_tcq_sxzt[curr_page] >= ddsc_32p_entry->curr_pack_id) // ����������� �ҵ�ǰ�鿴idС����������
		{
			if(getClusterPackDisconnectCount(curr_page, ddsc_32p_entry->curr_pack_id) != PackDisconnectCount) // �ж��Ƿ����
			{
				uint8_t buff[32];
				sprintf((char *)buff, "pack%d ״̬:����", ddsc_32p_entry->curr_pack_id);
				...
				if(ddsc_32p_entry->last_temperature != PACK_wendu_buf[curr_page][ddsc_32p_entry->curr_pack_id] || temp_force_fresh == 1)
				{
					...
					//SetTextInt32(screen_id_entry, 9, PACK_wendu_buf[curr_page][ddsc_32p_entry->curr_pack_id], 1, 2);//�¶���ʾ
...
//					{
//						SetControlForeColor(screen_id_entry, 9, 0xFB20); // Ԥ��
//					}
//					else
//					{
...
//				}
				...
				{
					ddsc_32p_entry->last_smoke_state = temp_co_concen; // �洢һ����̼��ֵ
					sprintf((char *)buff, "һ����̼:%dPPM", temp_co_concen);
...
//					{
//						SetControlForeColor(screen_id_entry, 17, 0xFB20); // Ԥ��
//					}
//					else if(ddsc_32p_entry->last_temperat_state == 2)
//					{
//						SetControlForeColor(screen_id_entry, 17, 0xF800); // ����Ԥ��
//					}
//				}
...
//						SetControlForeColor(screen_id_entry, 13, 0x0400); // ����
					...
					{
//						SetControlForeColor(screen_id_entry, 13, 0xFB20); // Ԥ��
						SetTextValue(screen_id_entry, 13, "��        ��:����");
					} 
					else if(PACK_YWZT_buf[curr_page][ddsc_32p_entry->curr_pack_id] == 2)
					{
//						SetControlForeColor(screen_id_entry, 13, 0xF800); // ����Ԥ��
						SetTextValue(screen_id_entry, 13, "��        ��:����");
					}
...
//					SetControlForeColor(screen_id_entry, 17, 0x8410);
					SetTextValue(screen_id_entry, 17, "һ����̼:--");
...
//				SetTextValue(screen_id_entry, 9, "��        ��:--");
		...
		else
		{
			...
			{
				uint8_t buff[32];
				sprintf((char *)buff, "pack%d ״̬:δ����", ddsc_32p_entry->curr_pack_id);
...
static void InternalScreenShowClusterData_32Pack_Plus(uint16_t screen_id_entry, DetectorDataShowCtrl_32Pack *ddsc_32p_entry)
{
	...
	if(screen_id_entry == 61) // �������̽����������ֵ�鿴����
	{
		for(uint8_t i = 1; i < 33; i++)
		{
			...
			else if(pack_online_buff[curr_page][i] == 1) // �������Ϊ������
			{
				if(getClusterPackDisconnectCount(curr_page, i) != PackDisconnectCount) // �ж��Ƿ����
				{
	...
	else if(screen_id_entry == 62) // �鿴����̽������ֵ����
	{
		...
		if(pack_online_buff[curr_page][check_pack_id] == 1) // �����������
		{
			if(getClusterPackDisconnectCount(curr_page, check_pack_id) != PackDisconnectCount)
			{
				...
				{
					sprintf((char *)buff, "pack%d ״̬:����", check_pack_id);
					SetTextValue(screen_id_entry, 5, buff);
					ddsc_32p_entry->last_derector_state[curr_page - 1][check_pack_id - 1] = 0; // ��ֵΪ0
				}
...
static void InternalScreenShowCabinDate(CabinDataShowCtrl_t *cabin_dsc_entry)
{
	...
	if(cang_sxzt[temp_cabin_id] != 0)
	{
		if(cang_sxzt[temp_cabin_id] == 1 || cabin_dsc_entry->force_fresh_flag == 1) // �������
		{
			...
			{
				uint8_t temp_buff[32] = {0};
				sprintf((char *)temp_buff,"̽����%d״̬:����", temp_cabin_id);
				SetTextValue(temp_screen_id, 2, temp_buff);
				SetTextValue(temp_screen_id, 3, "̽�����ͺ�:--");
				SetTextValue(temp_screen_id, 4, "����������״̬:--");
				SetTextValue(temp_screen_id, 5, "�¶�:--");
				SetTextValue(temp_screen_id, 6, "����״̬:--");
				SetTextValue(temp_screen_id, 7, "һ����̼Ũ��:--");
				SetTextValue(temp_screen_id, 8, "����Ũ��:--");
			}
			else
			{
				uint8_t temp_buff[32] = {0};
				...
				if(cabin_dsc_entry->last_detector_model != Cang_TCQXH_buf[temp_cabin_id] || cabin_dsc_entry->force_fresh_flag == 1)
				{
					...
					{
						case 1: {
							SetTextValue(temp_screen_id, 3, "̽�����ͺ�:XR805-V2.0");
							break;
						}
						case 2: {
							SetTextValue(temp_screen_id, 3, "̽�����ͺ�:XR805-EXD");
							break;
						}
						case 3: {
							SetTextValue(temp_screen_id, 3, "̽�����ͺ�:XR805-EXi");
							break;
						}
						default:
							SetTextValue(temp_screen_id, 3, "̽�����ͺ�:--");
...
//						SetTextValue(temp_screen_id, 4, temp_buff);
				...
				if(cabin_dsc_entry->last_co_value != Cang_COzhi_buf[temp_cabin_id] || cabin_dsc_entry->force_fresh_flag == 1)
				{
					...
					sprintf((char *)temp_buff, "һ����̼Ũ��:%dPPM", Cang_COzhi_buf[temp_cabin_id]);
				...
				if(cabin_dsc_entry->last_hh_value != Cang_H2zhi_buf[temp_cabin_id] || cabin_dsc_entry->force_fresh_flag == 1)
				{
					...
					sprintf((char *)temp_buff, "����Ũ��:%dPPM", Cang_H2zhi_buf[temp_cabin_id]);
	...
	else
	{
		...
		{
			uint8_t temp_buff[32] = {0};
			sprintf((char *)temp_buff,"̽����%d״̬:δ����", temp_cabin_id);
			SetTextValue(temp_screen_id, 2, temp_buff);
			SetTextValue(temp_screen_id, 3, "̽�����ͺ�:--");
			SetTextValue(temp_screen_id, 4, "����������״̬:--");
			SetTextValue(temp_screen_id, 5, "�¶�:--");
...
static void DetectorShowMaxCombusteGasValue(MaxCombustibleGas_t *mcg_entry)
{
	...
	if(max_combustible_gas_fresh_flag == 1)
	{
		...
		// ��������쳣
		if(temp_mcg.co_max_val == -1) // ��ʾû��̽�������� ��ȫ��̽��������
		{
			SetTextValue(temp_screen_id, 26, "̽����δ����/ȫ����/����ȷ����");
		}
		else
		{
			...
			if(temp_mcg.curr_da.cluster_id == 0 && temp_mcg.curr_da.pack_id == 0 && temp_mcg.curr_da.cabin_id != 0)
			{
				...
				{
					sprintf((char *)temp_buff, "��1��· %d�� ����Ũ��%dPPM", temp_mcg.curr_da.cabin_id, temp_mcg.co_max_val);
					SetTextValue(temp_screen_id, 26, temp_buff);
				}
				else
				{
					sprintf((char *)temp_buff, "��1��· %d�� һ����̼Ũ��%dPPM", temp_mcg.curr_da.cabin_id, temp_mcg.co_max_val);
					SetTextValue(temp_screen_id, 26, temp_buff);
				}
			...
			{
				sprintf((char *)temp_buff, "��%d��pack%dһ����̼Ũ��%dPPM", temp_mcg.curr_da.cluster_id, temp_mcg.curr_da.pack_id, temp_mcg.co_max_val);
				SetTextValue(temp_screen_id, 26, temp_buff);
			}
			else
			{
				SetTextValue(temp_screen_id, 26, "̽��������ȷ����");
			}
		}
...
static void DetectorDataFreshMenuCtrl_32Pack(DetectorDataShowCtrl_32Pack *ddsc_32p_entry, uint16_t ctrl_id, uint8_t item, uint8_t state)
{
	if(ctrl_id == 103)
	{
		if(state == 1) // ��������
		{
			...
			ddsc_32p_entry->force_fresh_flag = 1; // ǿ��ˢ�±�־λ
		}
	}
}
...
static void PointTypeDetectorOnlineIconCtrl(uint16_t ctrl_id, uint8_t state, uint8_t icon_num)
{
	...
	if(state == 1)
	{
		...
		AnimationPlayFrame(5, ctrl_id, set_online_state);//(����ID,�ؼ�ID,֡ID) 0�죬1��
...
static void PointTypeDetectorOnlineButtonCtrlPlus(uint16_t ctrl_id, uint8_t state)
{
	...
	switch(ctrl_id)
	{
		case 233: {
			...
			{
				PointTypeMixtureOnlieStateSingleSetting(i, 1);
				AnimationPlayFrame(5, i, 1);//(����ID,�ؼ�ID,֡ID) 0�죬1��
			}
			SavePointTypeSetOnlieState();
			ReadPointTypeSetOnlieState();
			break;
		...
		case 234: {
			for(uint8_t i = 1; i < 33; i++)
			{
				...
				AnimationPlayFrame(5, i, 0);//(����ID,�ؼ�ID,֡ID) 0�죬1��
			}
			SavePointTypeSetOnlieState();
			ReadPointTypeSetOnlieState();
			break;
			...
			if(temp_str_len != 0)
			{
				...
				if(success_len == 2 && x >0) // �����ɹ� ����x������
				{
					...
					if(getPointTypeMixtureDetectOnlineState(x) == 0)
					{
						...
						AnimationPlayFrame(5, x, 1);//(����ID,�ؼ�ID,֡ID) 0�죬1��
						modify_flag = 1;	
					}
					if(modify_flag == 1)
					{
...
static void PointTypeDetectorOnlineStateShowInit(void)
{
	for(uint8_t i = 1; i < 33; i++)
	{
		AnimationPlayFrame(5, i, getPointTypeMixtureDetectOnlineState(i));//(����ID,�ؼ�ID,֡ID) 0�죬1��
	}
}
...
void StoragePackCabinForeWarn(PackCabinForeWarnStorage *pcfws_entry, uint8_t cluster_id, uint8_t pack_id, uint8_t alarm_type)
{
	...
	for(uint8_t l = 0;l < pcfws_entry->self_bottom_point; l++)
	{
		...
		{
			// ����������Ĳ����кͽṹ������ͬ�� ����ѭ��
			if(pcfws_entry->da[l].pack_id == pack_id) // ����Ҫ�жϱ��������Ƿ�һ�� ��Ϊֻ����һ��
			{
				flag = 1;
				break;
			}
		...
		else if(pcfws_entry->da[l].pack_id != 0)
		{
			if( pcfws_entry->da[l].cabin_id == pack_id &&
					pcfws_entry->alarm_type[l]  == alarm_type) // ���ܵ���Ҳ�ܵ�pack id
			{
				flag = 1;
				break;
			}
...
void DeletPackCabinForeWarn(PackCabinForeWarnStorage *pcfws_entry, uint8_t cluster_id, uint8_t pack_id, uint8_t alarm_type)
{
	...
	if(flag == 1 && pcfws_entry->self_bottom_point > 0) // ��ʾ�ָ�������̽�����ڱ���������
	{
		...
		if(pcfws_entry->self_bottom_point > 0) // �ָ��������������Ļ�
		{
			beep_fire_ctrl |= 0x0F; // ��Ԥ��������
			silencers_state = 0; // ���µı��� �������� ���������־λ
		}
	}
}
	...
	uint8_t alarm_type)
{
	...
	{
		if(pcfas_entry->detector_class[l] == CabinClassID) // ����ǲֲŽ����ж�
		{
...
void StoragePackFireAlarm(
	PackCabinFireAlarmStorage *pcfas_entry, // Ҫ����Ļ�������ַ
	uint8_t cluster_id,                     // ��ID
	uint8_t pack_id,                        // ��/��
	uint8_t alarm_type                      // ��������
)
	...
	for(uint8_t l = 0;l < pcfas_entry->self_bottom_point; l++)
	{
		...
		else // ��IDΪ0(����)
		{
			if( pcfas_entry->da[l].cabin_id == pack_id &&
					pcfas_entry->alarm_type[l]  == alarm_type) // ���ܵ���Ҳ�ܵ�pack id
			{
				flag = 1;
				break;
			}
...
{
	uint8_t temp_screen_id = 59;
	uint8_t temp_partition = bkcnc_entry->curr_partition; // ����һ��
	...
	// ��Ҫ��ʼ��
	if(bkcnc_entry->curr_menu_state == InitMenu) // �ȳ�ʼ���˵�״̬
	{
		for(uint8_t i = 0; i < 3; i+=2)
		{
			clearTextValue(temp_screen_id, partition_ctrl_id[i]); // Ĭ����ռ�ͷ
			for(uint8_t j = 0; j < 7; j++)
			{
				clearTextValue(temp_screen_id, point_site_ctrl_id[i][j]); // Ĭ����ռ�ͷ
			}
		}
		bkcnc_entry->curr_menu_state = OutMenu; // Ĭ���ڲ˵���
	...
	switch(key_val_temp)
	{
		case KEY6_DIRECTION_UP   : // ����� ��
			if(bkcnc_entry->curr_menu_state == OutMenu) // �����ǰ���ڲ˵���
			{
				if(temp_partition == FireAlarmPart) // �����ǰ�����ǻ𾯷���
				{
					bkcnc_entry->curr_partition = ForceAlarmPart;
			...
			{
				if(temp_partition == ForceAlarmPart) // �������Ԥ����
				{
					if(pcfws.self_bottom_point > 0)
					{
						...
						{
							bkcnc_entry->curr_point_site[temp_partition]--; // ָ��ƫ��
						}
						else if(bkcnc_entry->curr_point_site[temp_partition] == 0) // ���Ϲ���
						{
				...
				else if(temp_partition == FaultPart) // ����ǹ�����
				{
					// ����й�����Ϣ
					if(pcfs_buttom_point > 0)
					{
...
//						}
				...
				else if(temp_partition == FireAlarmPart) // ����ǻ�
				{
					// ����й�����Ϣ
		...
		case KEY7_DIRECTION_RIGHT: // �������� �� ����
			if(temp_partition == ForceAlarmPart) // �����ǰ������Ԥ��
			{
				if(bkcnc_entry->curr_menu_state == InMenu) // ����ڲ˵���
				{
...
//					}
	...
	if(temp_partition != bkcnc_entry->last_partition) // 
	{
		...
		bkcnc_entry->last_partition = temp_partition; // �����Ѿ�����
	...
	// �˵���ָ�����
	if( bkcnc_entry->last_point_site[temp_partition] != bkcnc_entry->curr_point_site[temp_partition] || // ����Ƿ���״̬λ�øı�
			bkcnc_entry->last_menu_state                 != bkcnc_entry->curr_menu_state                    // ����ǲ˵�����״̬�ı�
	)
	{
		// �����ʷ��ͷ
...
{
	switch_ui_ctrl.curr_pack_alarm_len = &pas_pointer; // �󶨰�������
	...
	if(state == 1)
	{
		switch(ctrl_id)
		{
			case 1: {
				...
				SetTextInt32(57, 99, bsrr_entry->record_sum[bsrr_entry->curr_show_type], 0, 1);   //��¼������ʾ
				bsrr_entry->force_fresh_flag = 1; // ÿ�ν������ǿ��ˢ��һ��
...
//				BspReadFlashData(FAULT_FLASH_SAVE, read_data[3].byte_buff, 3);
			...
			case 2: {
				...
				SetTextInt32(56, 99, bsrr_entry->record_sum[bsrr_entry->curr_show_type], 0, 1);   //��¼������ʾ
				bsrr_entry->force_fresh_flag = 1; // ÿ�ν������ǿ��ˢ��һ��
		...
		bsrr_entry->curr_page[bsrr_entry->curr_show_type] = 1; // ÿ�ν���ǿ����ʾ��һҳ
	}
}
		...
		switch(ctrl_id)
		{
			case 1:
				if(bsrr_entry->curr_page[bsrr_entry->curr_show_type] > 1)
				{
					...
					bsrr_entry->force_fresh_flag = 1; // ǿ��ˢ�±�־λ
				}
				break;
			case 2:
...
static void InternalScreenShowRecord(BspScreenReadRecord_t *bsrr_entry)
{
	...
	uint8_t x_sector; // ����ƫ��
	uint8_t temp_page = bsrr_entry->curr_page[bsrr_entry->curr_show_type]; // ��ʱ��¼һ��ҳ Ԥ���ٱ𴦱��޸�
	
	if(bsrr_entry->force_fresh_flag == 1) // ǿ��ˢ�±�־λ
	{
		uint8_t show_buff[32];
		uint16_t temp_caculate;
		uint16_t data_index;
		uint16_t start = (temp_page - 1)*RECORD_SHOW_ZONE; // ��ʼ�±�
		...
		for(uint16_t i = 0; i < RECORD_SHOW_ZONE; i++)
		{
			...
			if(temp_caculate < total_records) // �����ǰ����С������
			{
				// �������� total_records = 591 �� ���� temp_page = 10 ��ʮҳ start = ��10-1��*10 = 90
				...
				switch(bsrr_entry->curr_show_type)
				{
					case RECORD_FAULT: {
						...
						if(read_data[x_sector].fs_sys_fault[data_index].fs_detect_id.cluster_id != 0)
						{
							...
							else if(read_data[x_sector].fs_sys_fault[data_index].fs_detect_id.cluster_id == LINKAGE_CLUSTER_ID)
							{
								switch(read_data[x_sector].fs_sys_fault[data_index].fs_detect_id.cabin_or_pack_id)
								{
									...
									case OutFir1_Package_ID: {
										sprintf((char *)show_buff, "���װ��1");
										break;
									}
									case OutFir2_Package_ID: {
										sprintf((char *)show_buff, "���װ��2");
										break;
									}
									case CabinBK_Package_ID: {
										sprintf((char *)show_buff, "���װ��");
										break;
									}
									case FEEDBK1_Package_ID: {
									...
									case HANDPOT_Package_ID: {
										sprintf((char *)show_buff, "�ֱ�");
										break;
									}
									case SYS_MAIN_POWER_KEY_ID: {
										sprintf((char *)show_buff, "����Դ");
										break;
									}
									case SYS_BACK_POWER_KEY_ID: {
										sprintf((char *)show_buff, "�����");
						...
						SetTextValue(temp_screen_id, device_ctrl_id[i], show_buff); //ˢ���豸����
						// ��ʾʱ��
						...
						sprintf((char *)show_buff, "%02d��%02d��%02d�� %02dʱ%02d��%02d��", temp_time_save.years + 2000, temp_time_save.months, 
							temp_time_save.days, temp_time_save.hours , temp_time_save.minute, temp_time_save.second);
						
						SetTextValue(temp_screen_id , retime_ctrl_id[i], show_buff); //ˢ��ʱ��
						// ״̬��ʾ
                        ...
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_CO_SENSOR_RECOVERY) { sprintf((char *)show_buff, "CO���������ϻָ�"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_H2_SENSOR_FAULT) { sprintf((char *)show_buff, "H2����������"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_H2_SENSOR_RECOVERY) { sprintf((char *)show_buff, "H2���������ϻָ�"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_VOC_SENSOR_FAULT) { sprintf((char *)show_buff, "VOC����������"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_VOC_SENSOR_RECOVERY) { sprintf((char *)show_buff, "VOC���������ϻָ�"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_CH4_SENSOR_FAULT) { sprintf((char *)show_buff, "CH4����������"); }
                        else if(read_data[x_sector].fs_sys_fault[data_index].state == RS485_CH4_SENSOR_RECOVERY) { sprintf((char *)show_buff, "CH4���������ϻָ�"); }
						SetTextValue(temp_screen_id, states_ctrl_id[i], show_buff); //ˢ��״̬
...
//						SetTextValue(1, 37, show_buff); //ˢ��״̬
//						
...
//						SetTextValue(1, 36, show_buff); //ˢ��״̬
//						
						// �豸�����ж�
						...
						else if(read_data[x_sector].fs_fire_alarm[data_index].fs_base.fs_detect_id.cluster_id == MBUS_CONTROL_FLASH_ID)
					{
						...
						SetTextValue(temp_screen_id, values_ctrl_id[i], show_buff); //ˢ��״̬
...
//						data_index = (total_records - 1 - temp_caculate)%500; // ������������
						...
						{
							case OUTFIRE1OPEN_1:
								sprintf((char *)show_buff, "���װ�õ�һ������");
								break;
							case OUTFIRE1CLOSE:
								sprintf((char *)show_buff, "���װ�ùر�");
								break;
							case OUTFIRE1OPEN_2:
								sprintf((char *)show_buff, "���װ�õڶ�������");
								break;
							case OUTFIRE1OPEN_3:
								sprintf((char *)show_buff, "���װ�õ���������");
								break;
							case OUTFIRE_1_START_DELAY:
								sprintf((char *)show_buff, "���װ�õ�һ����������ʱ��ʼ");
								break;
							case OUTFIRE_2_START_DELAY:
								sprintf((char *)show_buff, "���װ�õڶ�����������ʱ��ʼ");
								break;
							case OUTFIRE_3_START_DELAY:
								sprintf((char *)show_buff, "���װ�õ�������������ʱ��ʼ");
								break;
							case OUTFIRE_STOP  :
								sprintf((char *)show_buff, "���װ��ǿ��ֹͣ");
								break;
							case OUTFIRESTART_AGAIN:
								sprintf((char *)show_buff, "���װ���ٴ�����");
								break;
							case OUTFIRE_ST_PRESS:
								sprintf((char *)show_buff, "���װ��������������");
								break;
							case OUTFIRE_SP_PRESS:
								sprintf((char *)show_buff, "���װ��ֹͣ��������");
								break;
							case OUTFIRE_SL_PRESS:
								sprintf((char *)show_buff, "���ⰴ������");
								break;
							case OUTFIRE_OVER :
								sprintf((char *)show_buff, "���װ��������");
								break;
							case OTHER_PART1_TURN_AUTO:
								sprintf((char *)show_buff, "����1�л�Ϊ�Զ�");
								break;
							case OTHER_PART1_TURN_HAND:
								sprintf((char *)show_buff, "����1�л�Ϊ�ֶ�");
								break;
							case OTHER_PART2_TURN_AUTO:
								sprintf((char *)show_buff, "����2�л�Ϊ�Զ�");
								break;
							case OTHER_PART2_TURN_HAND:
								sprintf((char *)show_buff, "����2�л�Ϊ�ֶ�");
								break;
							case OTHER_SYS_TURN_HAND:
								sprintf((char *)show_buff, "���������л�Ϊ�ֶ�");
								break;
							case OTHER_SYS_TURN_AUTO:
								sprintf((char *)show_buff, "���������л�Ϊ�Զ�");
								break;
							case OUTFIRE_FEEDBACK1:
								sprintf((char *)show_buff, "����1����");
								break;
							default:
								sprintf((char *)show_buff,"δ֪״̬");
								break;
						}
...
//						
			...
			{
				clearTextValue(temp_screen_id , serial_ctrl_id[i]); //(����ID,�ؼ�ID��
				clearTextValue(temp_screen_id , device_ctrl_id[i]); //(����ID,�ؼ�ID��
				clearTextValue(temp_screen_id , retime_ctrl_id[i]); //(����ID,�ؼ�ID��
				clearTextValue(temp_screen_id , states_ctrl_id[i]); //(����ID,�ؼ�ID��
				if(bsrr_entry->curr_show_type == RECORD_ALARM)
				{
					clearTextValue(temp_screen_id , values_ctrl_id[i]); //(����ID,�ؼ�ID��
				}
			}
		} // forѭ��ˢ����ʾ������
	} // �ж��Ƿ�ˢ�µ�����
...
static void BspAlarmDataSaveApp(
	FlashReadCtrlId addr_type, // �洢��ַ
	FlashSaveType save_type,   // �洢����
	uint8_t cluster_id,        // �غ�
...
)
{
	...
	temp_data.fs_base.fs_detect_id.cabin_or_pack_id = pack_or_cabin; // packID �ز�Ϊ0����pack
	// ʱ�丳ֵ
	// 2025/11/19 15:39 �޸�
...
static void PowerManageCtrl(uint8_t main_power_state, uint8_t back_power_state)
{
	...
	if(main_power_state == 0 && (back_power_state == open_circuit || back_power_state == short_circuit))
	{
		...
		StorageEvent_LogPowerOff(); /* ��ϻ��:�ػ��¼�(EVT_POWER_OFF=121), ������ȫʧ */
	}
	else
	{
		// �ж�����
		...
		else if(main_power_state == 1 && main_power_alarm_flag == 1)
		{
			...
			// �رձ�λ������
			main_power_beep_ctrl &= ~(1U << 0);
	//				silencers_state  = 0;  // �ر�����ָʾ�� �رշ����� // ����ָʾ�Ʋ�Ӧ����Ϩ�� Ӧ���µĹ��ϵ�������ϵͳ��λ��Ϩ��
			// �洢��FLASH���ϴ洢���� �������
...
static void HandForceStartAnyCluster(FireExtinguishDeviceActionSave *fedas_entry, uint16_t ctrl_id, uint8_t state)
{
	if(state == 1)
	{
		...
		fedas_entry->pack_id[fedas_entry->self_point_len]    = 1; // ��IDĬ����1
		fedas_entry->fed_action[fedas_entry->self_point_len] = FIRE_EXTINGUISH_START_SPRAY_DELAY; // ������ʱ״̬
		fedas_entry->countdown_val[fedas_entry->self_point_len] = 30; // ��ʱʱ��30��
...
//		return;
	...
	if(baojingjishi - fan_start_ticks >= 2)
	{
		...
		{
			if(getSysHandAutoState() == KEY_MANUAL) // �ֶ�״̬��Ҫ�ֶ����� ���
			{
    ...
    for(uint8_t addr = 1U; addr <= MIXTURE_DEVICE_MAX_ADDR; addr++)
    {
        ...
        if(old_state != raw_state)
        {
            if(type == 6U)
            {
                ...
                if(old_state == 3U) FecbusReport_Fault(addr, DEV_TYPE_TEMPERATURE, 1, 0, 1); /* FECbus:�¶ȴ��������ϻָ� */
                ...
                else if(raw_state == 2U)
                {
                    ...
                    FecbusReport_Fire(addr, DEV_TYPE_TEMPERATURE, 1, 0); /* FECbus:�ϱ��¶Ȼ� */
                }
                else if(raw_state == 3U)
                {
                    ...
                    FecbusReport_Fault(addr, DEV_TYPE_TEMPERATURE, 1, 0, 0); /* FECbus:�¶ȴ��������� */
                }
            }
            else
            {
                ...
                if(old_state == 8U) FecbusReport_Fault(addr, DEV_TYPE_SMOKE, 1, 0, 1); /* FECbus:������Ⱦ���ϻָ� */
                if(old_state == 9U) FecbusReport_Fault(addr, DEV_TYPE_SMOKE, 1, 0, 1); /* FECbus:�������������ϻָ� */
                ...
                else if(raw_state == 2U)
                {
                    ...
                    FecbusReport_Fire(addr, DEV_TYPE_SMOKE, 1, 0); /* FECbus:�ϱ������� */
                }
                else if(raw_state == 8U)
                {
                    ...
                    FecbusReport_Fault(addr, DEV_TYPE_SMOKE, 1, 0, 0); /* FECbus:������Ⱦ���� */
                }
...
uint8_t last_point_type_found_online_state = 1; // Ĭ�ϱ���ˢ��һ��
static void PointTypeDetectorShowApp(PointTypeShowCtrl_t *ptsc_entry)
{
	uint8_t temp_screen_id = 67;
	
	ptsc_entry->curr_fresh_time_ctrl = osKernelGetTickCount(); // ϵͳ��ǰʱ���
	
	if(ptsc_entry->curr_fresh_time_ctrl - ptsc_entry->last_fresh_time_ctrl >= 3000 || ptsc_entry->poll_show_ctrl.key_perss_fresh != 0) // ����һˢ��
	{
		uint8_t show_addr;
		
		uint8_t found_online = 0;
		
		ptsc_entry->last_fresh_time_ctrl = ptsc_entry->curr_fresh_time_ctrl; // ��¼ˢ��ʱ��
		...
		if(ptsc_entry->poll_show_ctrl.key_perss_fresh == 'p')
		{
			...
			{
				ptsc_entry->poll_show_ctrl.poll_circuits_id--; // �������� ֱ���ҵ����ߵ�����
				...
				{
					found_online = 1;
					last_point_type_found_online_state = 1; // ����������߸�״̬��Ϊ1 ������ֹÿ�ζ�û������ʱ�ظ�ˢ����Ļ
		...
		if(found_online == 0 && last_point_type_found_online_state != 0) // ���û�з������ߵ�̽����
		{
			last_point_type_found_online_state = 0; // ��������
			ptsc_entry->poll_show_ctrl.poll_circuits_id = ptsc_entry->poll_show_ctrl.last_detector_id; // ���ϴ���ѯֵ��ʼ
			
			// ������Ļ��ʾ
			SetTextValue(temp_screen_id, 33, "���κ�̽��������"); //ˢ��״̬
			clearTextValue(temp_screen_id , 34); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 35); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 36); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 37); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 38); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 39); //(����ID,�ؼ�ID��
		}
		else if(found_online != 0) // ������̽�������� �Ÿ�����ʾ
		{
			uint8_t temp_detect_type;
			
			uint8_t temp_buff[64] = {0};
			
			if(getPointTypeMixtureDisconnectCount(show_addr) < MIXTURE_DEVICE_DISCONNECT_SUM) // ̽��������
			{
//				if(getPointTypeMixtureDetectSmokeMemory(show_addr) == 0 && getPointTypeMixtureDetectTempertureMemory(show_addr) == 0)
//				{
//					sprintf((char *)temp_buff, "̽����%d״̬:���� | ���¶ȱ��� | ����������", show_addr);
//				}
...
//					sprintf((char *)temp_buff, "̽����%d״̬:���� | �¶ȱ��� | ��������", show_addr);
//				}
				sprintf((char *)temp_buff, "̽����%d״̬:����", show_addr);
				SetTextValue(temp_screen_id, 33, temp_buff); //ˢ��״̬
				...
				{
					case 1: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:XR805-V2.0"); //ˢ��״̬
						break;
					}
					case 2: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:XR805-EXD"); //ˢ��״̬
						break;
					}
					case 3: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:XR805-EXi"); //ˢ��״̬
						break;
					}
					case 4: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:XR-DLYGWG"); //ˢ��״̬
						break;
					}
					case 5: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:JTY-XR800B"); //ˢ��״̬
						break;
					}
					case 6: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:JTY-ZDM-XR8002/C"); //ˢ��״̬
						break;
					}
					case 7: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:JTY-GD-XR8001AI"); //ˢ��״̬
						break;
					}
					default: {
						SetTextValue(temp_screen_id, 34, "̽�����ͺ�:--"); //ˢ��״̬
						break;
					}
				}
				...
				{
					SetTextValue(temp_screen_id, 35, "����������״̬:�޴���������");
				}
				else
				{
					uint8_t first_sensor = 1;  // ����Ƿ��ǵ�һ��������
				...
				if(sensor_enable_state & 0x20) // �ж��¶��Ƿ�����
				{
					sprintf((char *)temp_buff, "�¶�ֵ:%d��", getPointTypeMixtureReceiveData(PointTypeData_Temper, show_addr));
					SetTextValue(temp_screen_id, 36 + screen_show_id_offset, temp_buff);
					screen_show_id_offset++; // �޸ĵ�ַƫ�� 
				}
				if(sensor_enable_state & 0x01) // �ж������Ƿ�����
				{
					if(getPointTypeMixtureReceiveState(PointTypeData_Smoke, show_addr) == 1)
					{
						SetTextValue(temp_screen_id, 36 + screen_show_id_offset, "����״̬:����");
					}
					else
					{
						SetTextValue(temp_screen_id, 36 + screen_show_id_offset, "����״̬:����");
	...
	// ��ѯˢ��
	if(ptsc_entry->verb_show_ctrl.verb_detector_id == 0) // �����0 û�в�ѯID
	{
		ptsc_entry->verb_show_ctrl.verb_detector_id = 255; // ��������
		SetTextValue(temp_screen_id, 23, "�������ѯ̽����ID"); //ˢ��״̬
		clearTextValue(temp_screen_id , 24); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 25); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 26); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 27); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 28); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 29); //(����ID,�ؼ�ID��
		SetTextValue(temp_screen_id, 30, "�˴�����ID"); //ˢ��״̬
	}
	else if(ptsc_entry->verb_show_ctrl.verb_detector_id != 255) // �������ȷID���� �Ҳ��Ǹ������Ƶ�ֵ
	{
		...
		if(getPointTypeMixtureDetectOnlineState( show_addr ) == 1) // �������������
		{
			...
			if(getPointTypeMixtureDisconnectCount(show_addr) < MIXTURE_DEVICE_DISCONNECT_SUM)
			{
				...
				if(show_addr != ptsc_entry->verb_show_ctrl.last_detector_id || ptsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					sprintf((char *)temp_buff, "̽����%d״̬:����", show_addr);
					SetTextValue(temp_screen_id, 23, temp_buff); //ˢ��״̬
				}
				...
				if(ptsc_entry->verb_show_ctrl.lsat_detect_name != ptsc_entry->verb_show_ctrl.verb_detect_name || ptsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					{
						case 1: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:XR805-V2.0"); //ˢ��״̬
							break;
						}
						case 2: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:XR805-EXD"); //ˢ��״̬
							break;
						}
						case 3: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:XR805-EXi"); //ˢ��״̬
							break;
						}
						case 4: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:XR-DLYGWG"); //ˢ��״̬
							break;
						}
						case 5: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:JTY-XR800B"); //ˢ��״̬
							break;
						}
						case 6: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:JTY-ZDM-XR8002/C"); //ˢ��״̬
							break;
						}
						case 7: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:JTY-GD-XR8001AI"); //ˢ��״̬
							break;
						}
						default: {
							SetTextValue(temp_screen_id, 24, "̽�����ͺ�:--"); //ˢ��״̬
							break;
						}
					}
				}
				...
				if(ptsc_entry->verb_show_ctrl.last_sensor_state != ptsc_entry->verb_show_ctrl.verb_sensor_state || ptsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					{
						SetTextValue(temp_screen_id, 25, "����������״̬:�޴���������");
					}
					else
					{
						uint8_t first_sensor = 1;  // ����Ƿ��ǵ�һ��������
				...
				// ��������״̬��ʾֵ
				uint8_t screen_show_id_offset = 0;
				if(ptsc_entry->verb_show_ctrl.verb_sensor_state & 0x20) // �ж��¶��Ƿ�����
				{
					...
					if(ptsc_entry->verb_show_ctrl.verb_temper_value != ptsc_entry->verb_show_ctrl.last_temper_value || ptsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						sprintf((char *)temp_buff, "�¶�ֵ:%d��", ptsc_entry->verb_show_ctrl.verb_temper_value);
						SetTextValue(temp_screen_id, 26 + screen_show_id_offset, temp_buff);
						screen_show_id_offset++; // �޸ĵ�ַƫ�� 
					}
				}
				if(ptsc_entry->verb_show_ctrl.verb_sensor_state & 0x01) // �ж������Ƿ�����
				{
					...
					if(ptsc_entry->verb_show_ctrl.verb_smokes_state != ptsc_entry->verb_show_ctrl.last_smokes_state || ptsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						{
							SetTextValue(temp_screen_id, 26 + screen_show_id_offset, "����״̬:����");
						}
						else
						{
							SetTextValue(temp_screen_id, 26 + screen_show_id_offset, "����״̬:����");
		...
		{
			ptsc_entry->verb_show_ctrl.verb_detector_id = 255; // ��������
			SetTextValue(temp_screen_id, 23, "��̽����δ����"); //ˢ��״̬
			clearTextValue(temp_screen_id , 24); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 25); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 26); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 27); //(����ID,�ؼ�ID��
...
static void PointTypeDetectorTextInputCtrlApp(PointTypeShowCtrl_t *ptsc_entry, uint16 control_id, uint8 *str)
{
	...
	if(control_id == 30)    
	{
		...
		ptsc_entry->verb_show_ctrl.force_fresh_ctrl = 1; // ǿ��ˢ��һ��
	}
}
...
// ����̽����ˢ��
uint8_t last_composite_found_online_state = 1; // �����û���߿���ǿ��ˢ��һ��
void CompositeDetectorPollCtrl(CompositeShowCtrl_t *cpsc_entry)
{
	uint8_t temp_screen_id = 67;
	
	cpsc_entry->curr_fresh_time_ctrl = osKernelGetTickCount(); // ϵͳ��ǰʱ���
	
	if(cpsc_entry->curr_fresh_time_ctrl - cpsc_entry->last_fresh_time_ctrl >= 3000 || cpsc_entry->poll_show_ctrl.key_perss_fresh != 0) // ����һˢ��
	{
		uint8_t show_addr;
		
		uint8_t found_online = 0;
		
		cpsc_entry->last_fresh_time_ctrl = cpsc_entry->curr_fresh_time_ctrl; // ��¼ˢ��ʱ��
		...
		if(cpsc_entry->poll_show_ctrl.key_perss_fresh == 'p')
		{
			for(uint8_t test_addr = 0U; test_addr < MIXTURE_DEVICE_MAX_ADDR; test_addr++)
			{
				cpsc_entry->poll_show_ctrl.poll_circuits_id--; // �������� ֱ���ҵ����ߵ�����
				...
				{
					found_online = 1;
					last_composite_found_online_state = 1; // ����������߸�״̬��Ϊ1 ������ֹÿ�ζ�û������ʱ�ظ�ˢ����Ļ
		...
		if(found_online == 0 && last_composite_found_online_state != 0) // ���û�з������ߵ�̽����
		{
			last_composite_found_online_state = 0; // ��������
			cpsc_entry->poll_show_ctrl.poll_circuits_id = cpsc_entry->poll_show_ctrl.last_detector_id; // ���ϴ���ѯֵ��ʼ
			
			// ������Ļ��ʾ
			SetTextValue(temp_screen_id, 15, "���κ�̽��������"); //ˢ��״̬
			clearTextValue(temp_screen_id , 16); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 17); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 18); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 19); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 20); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 21); //(����ID,�ؼ�ID��
		}
		else if(found_online != 0) // ������̽�������� �Ÿ�����ʾ
		{
			uint8_t temp_buff[64] = {0};
			...
			// �˴��޸�Ϊ �жϸ��ϲ��Ƿ����
			if( Cang_zx_buf[ cpsc_entry->poll_show_ctrl.poll_circuits_id ] < CabinDisconnectCount ) // ̽��������
			{
//				if(getPointTypeMixtureDetectSmokeMemory(show_addr) == 0 && getPointTypeMixtureDetectTempertureMemory(show_addr) == 0)
//				{
//					sprintf((char *)temp_buff, "̽����%d״̬:���� | ���¶ȱ��� | ����������", show_addr);
...
//				}
				sprintf((char *)temp_buff, "̽����%d״̬:����", show_addr);
				SetTextValue(temp_screen_id, 15, temp_buff); //ˢ��״̬
				
				// ��ʾ̽�����ͺ� �˴��޸�Ϊ 805xxx����̽��������
				switch( Cang_TCQXH_buf[ show_addr ] )
				{
					case 1: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:XR805-V2.0"); //ˢ��״̬
						break;
					}
					case 2: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:XR805-EXD"); //ˢ��״̬
						break;
					}
					case 3: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:XR805-EXi"); //ˢ��״̬
						break;
					}
					case 4: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:XR-DLYGWG"); //ˢ��״̬
						break;
					}
					case 5: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:JTY-XR800B"); //ˢ��״̬
						break;
					}
					case 6: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:JTY-ZDM-XR8002/C"); //ˢ��״̬
						break;
					}
					case 7: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:JTY-GD-XR8001AI"); //ˢ��״̬
						break;
					}
					default: {
						SetTextValue(temp_screen_id, 16, "̽�����ͺ�:--"); //ˢ��״̬
						break;
					}
				}
				...
				if(sensor_enable_state  == 0 )
				{
					SetTextValue(temp_screen_id, 17, "����������״̬:�޴���������");
				}
				else
				{
					uint8_t first_sensor = 1;  // ����Ƿ��ǵ�һ��������
				...
				{
					// �˴��޸�Ϊ 805xxx����̽��������
					sprintf((char *)temp_buff, "�¶�ֵ:%d��", Cang_wendu_buf[ show_addr ] );
				...
				if(sensor_enable_state & 0x01) // �ж������Ƿ�����
				{
					...
					{
						SetTextValue(temp_screen_id, 18 + screen_show_id_offset, "����״̬:����");
					}
					else
					{
						SetTextValue(temp_screen_id, 18 + screen_show_id_offset, "����״̬:����");
				...
				if(sensor_enable_state & 0x10) // �ж�һ����̼�Ĵ����Ƿ�����
				{
					// �˴��޸�Ϊ 805xxx����̽��������
					sprintf((char *)temp_buff, "һ����̼Ũ��:%dPPM", Cang_COzhi_buf[ show_addr ] );
					SetTextValue(temp_screen_id, 18 + screen_show_id_offset, temp_buff);
					screen_show_id_offset++; // �޸ĵ�ַƫ�� 
				}
				if(sensor_enable_state & 0x04) // �ж������Ƿ�����
				{
					// �˴��޸�Ϊ 805xxx����̽��������
					sprintf((char *)temp_buff, "����Ũ��:%dPPM", Cang_H2zhi_buf[show_addr] );
					SetTextValue(temp_screen_id, 18 + screen_show_id_offset, temp_buff);
					screen_show_id_offset++; // �޸ĵ�ַƫ�� 
				}
			}
...
void CompositeDetectorVerbCtrl(CompositeShowCtrl_t *cpsc_entry)
{
	uint8_t temp_screen_id = 67;
	// ��ѯˢ��
	if(cpsc_entry->verb_show_ctrl.verb_detector_id == 0) // �����0 û�в�ѯID
	{
		cpsc_entry->verb_show_ctrl.verb_detector_id = 255; // ��������
		SetTextValue(temp_screen_id, 5, "�������ѯ̽����ID"); //ˢ��״̬
		clearTextValue(temp_screen_id , 6); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 7); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 8); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 9); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 10); //(����ID,�ؼ�ID��
		clearTextValue(temp_screen_id , 11); //(����ID,�ؼ�ID��
		
		SetTextValue(temp_screen_id, 12, "�˴�����ID"); //ˢ��״̬
	}
	else if(cpsc_entry->verb_show_ctrl.verb_detector_id != 255) // �������ȷID���� �Ҳ��Ǹ������Ƶ�ֵ
	{
		...
		if( cang_sxzt[ show_addr ] == 1 ) // �������������
		{
			...
			if( Cang_zx_buf[ show_addr ] < CabinDisconnectCount )
			{
				...
				if(show_addr != cpsc_entry->verb_show_ctrl.last_detector_id || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					sprintf((char *)temp_buff, "̽����%d״̬:����", show_addr);
					SetTextValue(temp_screen_id, 5, temp_buff); //ˢ��״̬
				}
				...
				if(cpsc_entry->verb_show_ctrl.lsat_detect_name != cpsc_entry->verb_show_ctrl.verb_detect_name || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					{
						case 1: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:XR805-V2.0"); //ˢ��״̬
							break;
						}
						case 2: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:XR805-EXD"); //ˢ��״̬
							break;
						}
						case 3: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:XR805-EXi"); //ˢ��״̬
							break;
						}
						case 4: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:XR-DLYGWG"); //ˢ��״̬
							break;
						}
						case 5: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:JTY-XR800B"); //ˢ��״̬
							break;
						}
						case 6: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:JTY-ZDM-XR8002/C"); //ˢ��״̬
							break;
						}
						case 7: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:JTY-GD-XR8001AI"); //ˢ��״̬
							break;
						}
						default: {
							SetTextValue(temp_screen_id, 6, "̽�����ͺ�:--"); //ˢ��״̬
							break;
						}
					}
				}
				...
				if(cpsc_entry->verb_show_ctrl.last_sensor_state != cpsc_entry->verb_show_ctrl.verb_sensor_state || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
				{
					...
					{
						SetTextValue(temp_screen_id, 7, "����������״̬:�޴���������");
					}
					else
					{
						uint8_t first_sensor = 1;  // ����Ƿ��ǵ�һ��������
				...
				// ��������״̬��ʾֵ
				uint8_t screen_show_id_offset = 0;
				if(cpsc_entry->verb_show_ctrl.verb_sensor_state & 0x20) // �ж��¶��Ƿ�����
				{
					...
					if(cpsc_entry->verb_show_ctrl.verb_temper_value != cpsc_entry->verb_show_ctrl.last_temper_value || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						sprintf((char *)temp_buff, "�¶�ֵ:%d��", cpsc_entry->verb_show_ctrl.verb_temper_value);
						SetTextValue(temp_screen_id, 8 + screen_show_id_offset, temp_buff);
						screen_show_id_offset++; // �޸ĵ�ַƫ�� 
					}
				}
				if(cpsc_entry->verb_show_ctrl.verb_sensor_state & 0x01) // �ж������Ƿ�����
				{
					...
					if(cpsc_entry->verb_show_ctrl.verb_smokes_state != cpsc_entry->verb_show_ctrl.last_smokes_state || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						{
							SetTextValue(temp_screen_id, 8 + screen_show_id_offset, "����״̬:����");
						}
						else
						{
							SetTextValue(temp_screen_id, 8 + screen_show_id_offset, "����״̬:����");
				...
				if(cpsc_entry->verb_show_ctrl.verb_sensor_state & 0x10) // �ж�һ����̼�Ĵ����Ƿ�����
				{
					// �˴��޸�Ϊ 805xxx����̽��������
					...
					if(cpsc_entry->verb_show_ctrl.last_carbon_value != cpsc_entry->verb_show_ctrl.verb_carbon_value || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						sprintf((char *)temp_buff, "һ����̼Ũ��:%dPPM", cpsc_entry->verb_show_ctrl.verb_carbon_value );
						SetTextValue(temp_screen_id, 8 + screen_show_id_offset, temp_buff);
						screen_show_id_offset++; // �޸ĵ�ַƫ�� 
					}
				}
				if(cpsc_entry->verb_show_ctrl.verb_sensor_state & 0x04) // �ж������Ƿ�����
				{
					...
					if(cpsc_entry->verb_show_ctrl.lsat_hydrog_value != cpsc_entry->verb_show_ctrl.verb_hydrog_value || cpsc_entry->verb_show_ctrl.force_fresh_ctrl == 1)
					{
						...
						// �˴��޸�Ϊ 805xxx����̽��������
						sprintf((char *)temp_buff, "����Ũ��:%dPPM", cpsc_entry->verb_show_ctrl.verb_hydrog_value );
						SetTextValue(temp_screen_id, 8 + screen_show_id_offset, temp_buff);
						screen_show_id_offset++; // �޸ĵ�ַƫ�� 
					}
				}
		...
		{
			cpsc_entry->verb_show_ctrl.verb_detector_id = 255; // ��������
			SetTextValue(temp_screen_id, 5, "��̽����δ����"); //ˢ��״̬
			clearTextValue(temp_screen_id , 6); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 7); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 8); //(����ID,�ؼ�ID��
			clearTextValue(temp_screen_id , 9); //(����ID,�ؼ�ID��
...
static void CompositeDetectorTextInputCtrlApp(CompositeShowCtrl_t *cpsc_entry, uint16 control_id, uint8 *str)
{
	...
	if(control_id == 12)    
	{
		...
		cpsc_entry->verb_show_ctrl.force_fresh_ctrl = 1; // ǿ��ˢ��һ��
	}
}
...
UART_HandleTypeDef *getSimulateSirealPortSendHandle(uint8_t port_comid)
{
	...
	switch(port_comid)
	{
		...
		case 6:{
			uart_handle = NULL; /* XR5000_UART5_EXCLUSIVE_FIX_20260730 */ // �������ߴ���
			break;
		}
		default:{
			uart_handle = NULL;
...
eUartOrder getSimulateSirealPortReceiveIndex(uint8_t port_comid)
{
	...
	switch(port_comid)
	{
		...
		case 5:{
			temp_order = STATION_OPTICALFIBER; // ��վ����
			break;
		}
		case 6:{
			temp_order = ERRORSITE; /* XR5000_UART5_EXCLUSIVE_FIX_20260730 */ // �������ߴ���
			break;
		}
		default:{
...
static void SimulationSerialPortFirstFresh(SimulationSerialPortAssistant_t *sspa_entry)
{
	...
	{
		SetTextValue(3, 9, "�򿪴���"); //ˢ��״̬
	}
	else
	{
		SetTextValue(3, 9, "�رմ���"); //ˢ��״̬
	}
	
	if(sspa_entry->serial_port_comid == 0 || sspa_entry->serial_port_comid == 0xFF)
	{
		clearTextValue(3, 6); //(����ID,�ؼ�ID��
	}
	
	if(sspa_entry->serial_port_send_mode == 0)
	{
		SetTextValue(3, 32, "�ַ�������"); //ˢ��״̬
	}
	else
	{
		SetTextValue(3, 32, "16���Ʒ���"); //ˢ��״̬
	}
	
	if(sspa_entry->serial_port_show_mode == 0)
	{
		SetTextValue(3, 36, "�ַ�������"); //ˢ��״̬
	}
	else
	{
		SetTextValue(3, 36, "16���ƽ���"); //ˢ��״̬
	}
	
	if(sspa_entry->serial_port_send_new_row == 0)
	{
		SetTextValue(3, 38, "����������"); //ˢ��״̬
	}
	else
	{
		SetTextValue(3, 38, "��������"); //ˢ��״̬
...
static void SimulationSerialPortButtonCtrl(SimulationSerialPortAssistant_t *sspa_entry, uint16_t ctrl_id, uint8_t state)
{
	...
	{
		case 10: { // �򿪴��ڰ���
			if(sspa_entry->serial_port_state == 0) // ������ڴ����ǹر�״̬
			{
				if(sspa_entry->serial_port_comid != 0xFF && sspa_entry->serial_port_comid != 6U) // ����˿ں���ȷ
				{
					eUartOrder temp_order = ERRORSITE;
					temp_order = getSimulateSirealPortReceiveIndex(sspa_entry->serial_port_comid);
					if(temp_order != ERRORSITE) // �������ȷ��ID
					{
						uartbuff[temp_order].recepetion_flag = 0;
						memset(uartbuff[temp_order].recepetion_buff, 0, BUFF_MAX);
					}
					sspa_entry->serial_port_state = 1; // �򿪴���
					SuspendTask(sspa_entry->serial_port_comid); // �����Ӧ������ʹ���ڿ���
					SetTextValue(3, 9, "�رմ���"); //ˢ��״̬
				}
			}
			else
			{
				...
				sspa_entry->serial_port_state = 0; // �رմ���
				ResumeTask(sspa_entry->serial_port_comid); // �ָ����� 
				SetTextValue(3, 9, "�򿪴���"); //ˢ��״̬
			}
			break;
		}
		case 14: { // ���Ͱ���
			if(sspa_entry->serial_port_state == 1) // ��������Ǵ򿪵�
			{
				if(sspa_entry->serial_port_send_len != 0) // ������ͻ�������ȡ�ĳ��Ȳ�����0
				{
...
static void SimulationSerialPortScreenShowApp(SimulationSerialPortAssistant_t *sspa_entry)
{
	...
	if(sspa_entry->serial_port_state == 1) // ��������Ǵ򿪵�
	{
		...
		{
			if(uartbuff[temp_order].recepetion_flag == 1) // ������յ���
			{
				uartbuff[temp_order].recepetion_flag = 0; // ��ս���
				
				if(sspa_entry->serial_port_show_mode == 1) // 16������ʾ
				{
					uint8_t serial_port_receive_buff[256]; // ��ʾ������ ���������ݱ��16����
					HexToHexStringLight(uartbuff[temp_order].recepetion_buff, uartbuff[temp_order].recepetion_len, serial_port_receive_buff);
					
					SetTextValue(3, 15 + sspa_entry->serial_port_show_offset, serial_port_receive_buff); //ˢ��״̬
				...
				else
				{
					...
					while (token != NULL) {
							SetTextValue(3, 15 + sspa_entry->serial_port_show_offset, (uint8_t *)token); //ˢ��״̬
...
)
{
	if(pcfws_entry->self_bottom_point != 0 && sicj_entry->warn_fresh_flag == 0)
	{
		...
		sicj_entry->warn_fresh_flag = 1; // ��־����״̬ˢ�����
		// ��һ��������Ϣ�ö���ʾ
		if(pcfws_entry->detector_class[0] == PackClassID && pcfws_entry->da[0].cluster_id == RS485_DETECT_FLASH_ID)
		{
			// XR5000_LOOP3_CHANGE_20260726: Loop 3 first warning display uses "��3��· X��".
		...
		else if(pcfws_entry->detector_class[0] == PackClassID)
		{
			if(pcfws_entry->alarm_type[0] == Temperature)
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽�����¶ȱ���", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cluster_id, pcfws_entry->da[0].pack_id);
			}
			...
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽������������", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws.atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws.da[0].cluster_id, pcfws.da[0].pack_id);
			}
			...
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d ��%d��%d��PACK̽����һ����̼����", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cluster_id, pcfws_entry->da[0].pack_id);
			}
		...
		else if(pcfws_entry->detector_class[0] == CabinClassID)
		{
			if(pcfws_entry->alarm_type[0] == Temperature)
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� �¶ȱ���", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cabin_id);
			}
			...
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d ��1��· %d�� һ����̼����", 1,
				pcfws_entry->atr[0].years, pcfws_entry->atr[0].months, pcfws_entry->atr[0].days,
				pcfws_entry->atr[0].hours, pcfws_entry->atr[0].minute, pcfws_entry->atr[0].second,
				pcfws_entry->da[0].cabin_id);
			}
		...
		{
			SetTextValue(screen_top_fresh_id[i], warn_alarm_fresh_id[i], temp_buff); // ˢ�µ�һ����������
		}
	...
	else if(pcfws_entry->self_bottom_point == 0 && sicj_entry->warn_fresh_flag != 0)
	{
		for(uint8_t i = 0; i < 4; i++)
		{
			clearTextValue(screen_top_fresh_id[i], warn_alarm_fresh_id[i]); // ˢ�µ�һ����������
		}
	...
	if(pcfas_entry->self_bottom_point != 0 && sicj_entry->fire_fresh_flag == 0)
	{
		...
		else if(pcfas_entry->detector_class[0] == LinkageClassID)
		{
			...
			{
				sprintf((char*)temp_buff, "%03d %d/%02d/%02d %02d:%02d:%02d �ֱ����±���", 1,
					pcfas_entry->atr[0].years, pcfas_entry->atr[0].months, pcfas_entry->atr[0].days,
					pcfas_entry->atr[0].hours, pcfas_entry->atr[0].minute, pcfas_entry->atr[0].second);
			}
		...
		for(uint8_t i = 0; i < 4; i++)
		{
			SetTextValue(screen_top_fresh_id[i], fire_alarm_fresh_id[i], temp_buff); // ˢ�µ�һ����������
		}
	}
	else if(pcfas_entry->self_bottom_point == 0 && sicj_entry->fire_fresh_flag != 0)
	{
		for(uint8_t i = 0; i < 4; i++)
		{
			clearTextValue(screen_top_fresh_id[i], fire_alarm_fresh_id[i]); // ˢ�µ�һ����������
		}
...
//	SystemSaveInfo.last_license_store[0] = '\0'; // ֮ǰ�洢����Ȩ��
