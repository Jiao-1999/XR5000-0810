...
#include "bsp_can_monitor.h" /* ???????FCP-1011??¡¤????‰Ù???2026-08-06 */
#include "bsp_aht20.h"
#include "iwdg.h"
#include "bsp_storage_tx.h"  /* ?›¥???: LPUART1(PB6/PB7) */
#include "bsp_fecbus.h"      /* FECbus RS485 ???: USART3(PB10/PB11), GB4717 ???C */
#include "bsp_test_inject.h" /* COM4??????????: UART4 RX?????, ??DMA?§Ø?????§µ?? */
...
// 2025/11/20 18:11 ???? ????????? ??????????? ???????????
osThreadId_t CanMonitorTaskHandle; /* ???????FCP-1011??¡¤????‰Ù???2026-08-06 */
...
const osThreadAttr_t FecbusTxTask_attributes = {
	...
	.priority = (osPriority_t) osPriorityBelowNormal1,  /* ????StorageTx, ????????›¥??????? */
};
...
const osThreadAttr_t FecbusPeriodicTask_attributes = {
	...
	.stack_size = 256 * 4,  /* ?????????§³: ????????payload[7] */
	.priority = (osPriority_t) osPriorityLow,  /* ???????????????, ????????????? */
};

/* COM4???????????: UART4 RX?????DMA????, ????ONL/TF/SF???????????????§Ø? */
...
const osThreadAttr_t defaultTask_attributes = {
  .name = "defaultTask",
  .stack_size = 512 * 4,  /* ???§³: DebugPrintf?400??? + vsnprintf??????? */
  .priority = (osPriority_t) osPriorityNormal,
};
...
//	xMutex = xSemaphoreCreateMutexStatic(&xMutexBuffer); // ???????????
  ...
  /* add queues, ... */
	xMyRs485QueueHandle = xQueueCreate(10, sizeof(char[8])); // ??????????›¥10?????????§µ?????????8????char????
	...
	xMyMBusQueueHandle = xQueueCreate( 5, sizeof(char[8])); // ??????????›¥5?????????§µ?????????8????char????

	xMBus2QueueHandle = xQueueCreate( 5, sizeof(char[8])); // ??????????›¥5?????????§µ?????????8????char???÷Ï??¡¤2 ???????
...
//	CtrlBusPollAndReceiveTaskHandle = osThreadNew(CtrlBusPollAndReceiveTask, NULL, &CtrlBusPollAndReceiveTask_attributes);
	...
	// 2025/11/20 18:12
	CanMonitorTaskHandle = osThreadNew(CanMonitorTask, NULL, &CanMonitorTask_attributes); /* ???????FCP-1011??¡¤????‰Ù???2026-08-06 */
...
void SuspendTask(uint8_t task_id)
{
	switch(task_id)
	{
		...
		case 4:{
//                 vTaskSuspend(BMSRecvDealTaskHandle); // ????BMS???? (FECBUS_TEST: BMS????¦Ä????, handle=NULL)
			break;
		}
		case 5:{
//			vTaskSuspend(StationResponseTaskHandle); // ???? ??? ???? (FECbus ????: ???????¦Ä???? 20260818)
			break;
		}
		case 6:{
...
void ResumeTask(uint8_t task_id)
{
	switch(task_id)
	{
		...
		case 4:{
//                 vTaskResume(BMSRecvDealTaskHandle); // ???BMS???? (FECBUS_TEST: BMS????¦Ä????, handle=NULL)
			break;
		}
		case 5:{
//			vTaskResume(StationResponseTaskHandle); // ??? ??? ???? (FECbus ????: ???????¦Ä???? 20260818)
			break;
		}
		case 6:{
...
/* FECbus ??????????: GB4717 ???C, ??USART3(PB10/PB11) RS485 §¿??? */
void FecbusTxTask(void *argument)
{
	/* ?????FECbus§¿???: ?????????/???????? + ????USART3???IT?§Ø?
	 * ?: Fecbus_Init ???????, ????????? */
