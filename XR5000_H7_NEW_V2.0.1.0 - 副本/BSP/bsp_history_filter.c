...
static FlashReadCache_t g_history_sector_cache; /* 报警与故障查询共用，避免重复占用约4KB静态RAM。 */
...
static uint8_t AlarmFilter_ScanAndDisplay(void)
{
    ...
    for(sector = sector_count; sector > 0U; sector--)
    {
        ...
        if(BspReadFlashData(FIRE_FLASH_SAVE, g_history_sector_cache.byte_buff, (uint8_t)sector_index) < 0)
        {
            SetTextValue(ALARM_FILTER_SCREEN_ID, ALARM_FILTER_MESSAGE_ID, (uint8_t *)TEXT_READ_FAILED);
            return 0U;
        }
        ...
        {
            FlashSaveFireAlarm_t *record = &g_history_sector_cache.fs_fire_alarm[absolute - first_index - 1U];
...
#define FAULT_FILTER_MAX_SECTORS          4U
#define FAULT_FILTER_DATE_FAULT_TEXT_ID         5U
#define FAULT_FILTER_MESSAGE_ID           61U
#define FAULT_FILTER_PAGE_ID              97U
#define FAULT_FILTER_TOTAL_ID             99U

#define FAULT_FILTER_LOOP_ALL             0U
#define FAULT_FILTER_LOOP_SYSTEM          4U

#define FAULT_FILTER_TYPE_ALL             0U
#define FAULT_FILTER_TYPE_POWER           1U
#define FAULT_FILTER_TYPE_COMMUNICATION   2U
#define FAULT_FILTER_TYPE_SENSOR          3U
#define FAULT_FILTER_TYPE_OTHER           4U

#define FAULT_TEXT_ALL_LOOPS       "\xC8\xAB\xB2\xBF\xBB\xD8\xC2\xB7"
#define FAULT_TEXT_LOOP1           "\xBB\xD8\xC2\xB7" "1"
#define FAULT_TEXT_LOOP2           "\xBB\xD8\xC2\xB7" "2"
#define FAULT_TEXT_LOOP3           "\xBB\xD8\xC2\xB7" "3"
#define FAULT_TEXT_SYSTEM          "\xCF\xB5\xCD\xB3"
#define FAULT_TEXT_ALL_FAULTS      "\xC8\xAB\xB2\xBF\xB9\xCA\xD5\xCF"
#define FAULT_TEXT_POWER_FAULT     "\xB5\xE7\xD4\xB4\xB9\xCA\xD5\xCF"
#define FAULT_TEXT_COMM_FAULT      "\xCD\xA8\xD0\xC5\xB9\xCA\xD5\xCF"
#define FAULT_TEXT_SENSOR_FAULT    "\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"
#define FAULT_TEXT_OTHER_FAULT     "\xC6\xE4\xCB\xFB\xB9\xCA\xD5\xCF"
#define FAULT_TEXT_QUERYING        "\xD5\xFD\xD4\xDA\xB2\xE9\xD1\xAF\xA3\xAC\xC7\xEB\xC9\xD4\xBA\xF2"
#define FAULT_TEXT_QUERY_DONE      "\xB2\xE9\xD1\xAF\xCD\xEA\xB3\xC9"
#define FAULT_TEXT_NO_RECORD       "\xCE\xB4\xB2\xE9\xD1\xAF\xB5\xBD\xB7\xFB\xBA\xCF\xCC\xF5\xBC\xFE\xB5\xC4\xB9\xCA\xD5\xCF\xBC\xC7\xC2\xBC"
#define FAULT_TEXT_DATE_FORMAT     "\xCA\xB1\xBC\xE4\xB8\xF1\xCA\xBD\xA3\xBA" "YYYYMMDD.YYYYMMDD"
#define FAULT_TEXT_DATE_INVALID    "\xC7\xEB\xCA\xE4\xC8\xEB\xD3\xD0\xD0\xA7\xC8\xD5\xC6\xDA"
#define FAULT_TEXT_DATE_ORDER      "\xBD\xE1\xCA\xF8\xC8\xD5\xC6\xDA\xB2\xBB\xC4\xDC\xD4\xE7\xD3\xDA\xBF\xAA\xCA\xBC\xC8\xD5\xC6\xDA"
#define FAULT_TEXT_READ_FAILED     "Flash\xB9\xCA\xD5\xCF\xBC\xC7\xC2\xBC\xB6\xC1\xC8\xA1\xCA\xA7\xB0\xDC"
...
static FaultFilterContext g_fault_filter;


static const uint16_t g_fault_serial_ids[FAULT_FILTER_ROWS] = {6U, 7U, 8U, 9U, 10U, 11U, 12U, 13U, 14U, 15U};
static const uint16_t g_fault_device_ids[FAULT_FILTER_ROWS] = {16U, 17U, 18U, 19U, 20U, 21U, 22U, 23U, 24U, 25U};
...
/* 清除四列结果，避免进入页面或更换筛选条件后残留上次内容。 */
static void FaultFilter_ClearRows(void)
{
    uint8_t i;
    for(i = 0U; i < FAULT_FILTER_ROWS; i++)
    {
        clearTextValue(FAULT_FILTER_SCREEN_ID, g_fault_serial_ids[i]);
        clearTextValue(FAULT_FILTER_SCREEN_ID, g_fault_device_ids[i]);
        clearTextValue(FAULT_FILTER_SCREEN_ID, g_fault_time_ids[i]);
        clearTextValue(FAULT_FILTER_SCREEN_ID, g_fault_state_ids[i]);
    }
}
...
static void FaultFilter_ResetResult(void)
{
    ...
    g_fault_filter.page = 1U;
    g_fault_filter.matched_count = 0U;
    FaultFilter_ClearRows();
    SetTextInt32(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_PAGE_ID, 1U, 0U, 1U);
    SetTextInt32(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_TOTAL_ID, 0U, 0U, 1U);
...
static uint8_t FaultFilter_ParseDatePart(const char *text, uint32_t *date_key)
{
    ...
    if(year < 2000U || year > 2255U || month == 0U || month > 12U) return 0U;
    max_day = month_days[month - 1U];
    if(month == 2U && FaultFilter_IsLeapYear(year)) max_day = 29U;
    if(day == 0U || day > max_day) return 0U;
    *date_key = (uint32_t)year * 10000UL + (uint32_t)month * 100UL + day;
    return 1U;
}
...
static uint8_t FaultFilter_ValidateDate(void)
{
    ...
    g_fault_filter.date_enabled = 0U;
    if(len == 0U || strcmp(g_fault_filter.date_text, "YYYYMMDD.YYYYMMDD") == 0) return 1U;
    if(len != 17U || g_fault_filter.date_text[8] != '.')
    {
        SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_DATE_FORMAT);
        return 0U;
    }
    if(!FaultFilter_ParseDatePart(g_fault_filter.date_text, &g_fault_filter.date_begin) ||
       !FaultFilter_ParseDatePart(&g_fault_filter.date_text[9], &g_fault_filter.date_end))
    {
        SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_DATE_INVALID);
        return 0U;
    }
    if(g_fault_filter.date_end < g_fault_filter.date_begin)
    {
        SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_DATE_ORDER);
        return 0U;
    }
    g_fault_filter.date_enabled = 1U;
    return 1U;
}
...
static uint8_t FaultFilter_IsPower(const FlashSaveDetectFault_t *record)
{
    ...
    return (uint8_t)(record->fs_detect_id.cluster_id == LINKAGE_CLUSTER_ID &&
...
static uint8_t FaultFilter_IsSensorState(uint8_t state)
{
    switch(state)
    {
        case RS485_TEMP_SENSOR_FAULT:
        case RS485_TEMP_SENSOR_RECOVERY:
        case RS485_SMOKE_POLLUTION_FAULT:
        case RS485_SMOKE_POLLUTION_RECOVERY:
        case LOOP1_TEMP_SENSOR_FAULT:
        case LOOP1_TEMP_SENSOR_RECOVERY:
        case LOOP1_SMOKE_POLLUTION_FAULT:
        case LOOP1_SMOKE_POLLUTION_RECOVERY:
        case LOOP1_SMOKE_SENSOR_FAULT:
        case LOOP1_SMOKE_SENSOR_RECOVERY:
        case RS485_CO_SENSOR_FAULT:
        case RS485_CO_SENSOR_RECOVERY:
        case RS485_H2_SENSOR_FAULT:
        case RS485_H2_SENSOR_RECOVERY:
        case RS485_VOC_SENSOR_FAULT:
        case RS485_VOC_SENSOR_RECOVERY:
        case RS485_CH4_SENSOR_FAULT:
        case RS485_CH4_SENSOR_RECOVERY:
            return 1U;
        default:
            return 0U;
    }
...
static uint8_t FaultFilter_GetType(const FlashSaveDetectFault_t *record)
{
    ...
    if(FaultFilter_IsLoopDevice(record) &&
       (record->state == DISCONNECT || record->state == DIS_RECOVERY)) return FAULT_FILTER_TYPE_COMMUNICATION;
    if(FaultFilter_IsSensorState(record->state)) return FAULT_FILTER_TYPE_SENSOR;
...
{
    uint8_t cluster = record->fs_detect_id.cluster_id;
    if(g_fault_filter.loop_filter == FAULT_FILTER_LOOP_ALL) return 1U;
    if(g_fault_filter.loop_filter == 1U) return (uint8_t)(cluster == 0U);
    if(g_fault_filter.loop_filter == 2U) return (uint8_t)(cluster == MBUS_CONTROL_FLASH_ID);
    if(g_fault_filter.loop_filter == 3U) return (uint8_t)(cluster == RS485_DETECT_FLASH_ID);
    if(g_fault_filter.loop_filter == FAULT_FILTER_LOOP_SYSTEM) return (uint8_t)(cluster == LINKAGE_CLUSTER_ID);
    return 0U;
}
...
static uint8_t FaultFilter_MatchDate(const FlashSaveDetectFault_t *record)
{
    FlashSaveTime_t time_value = {0};
    uint32_t key;
    if(!g_fault_filter.date_enabled) return 1U;
    getFlashTime_Plus(record->fs_time_buff, &time_value);
    key = (uint32_t)(time_value.years + 2000U) * 10000UL + (uint32_t)time_value.months * 100UL + time_value.days;
...
static uint8_t FaultFilter_Match(const FlashSaveDetectFault_t *record)
{
    ...
    return (uint8_t)(FaultFilter_MatchLoop(record) && FaultFilter_MatchDate(record) &&
...
static void FaultFilter_FormatDevice(const FlashSaveDetectFault_t *record, uint8_t *buffer)
{
    ...
    else if(cluster == LINKAGE_CLUSTER_ID)
    {
        switch(id)
        {
            case SYS_MAIN_POWER_KEY_ID: sprintf((char *)buffer, "\xD6\xF7\xB5\xE7\xD4\xB4"); break;
            case SYS_BACK_POWER_KEY_ID: sprintf((char *)buffer, "\xB1\xB8\xB5\xE7\xB3\xD8"); break;
            case SYS_FLASH_FAULT_ID: sprintf((char *)buffer, "Flash"); break;
            case SoundLt_Package_ID: sprintf((char *)buffer, "\xC9\xF9\xB9\xE2\xB1\xA8\xBE\xAF\xC6\xF7"); break;
            case SirenBk_Package_ID: sprintf((char *)buffer, "\xBE\xAF\xB5\xD1/\xB1\xB8\xD3\xC3"); break;
            case OutFir1_Package_ID: sprintf((char *)buffer, "\xC3\xF0\xBB\xF0\xD7\xB0\xD6\xC3" "1"); break;
            case OutFir2_Package_ID: sprintf((char *)buffer, "\xC3\xF0\xBB\xF0\xD7\xB0\xD6\xC3" "2"); break;
            default: sprintf((char *)buffer, "\xCF\xB5\xCD\xB3\xC9\xE8\xB1\xB8 %u", id); break;
        }
    }
    else sprintf((char *)buffer, "\xCE\xB4\xD6\xAA\xC9\xE8\xB1\xB8");
}
...
static void FaultFilter_FormatState(const FlashSaveDetectFault_t *record, uint8_t *buffer)
{
    ...
    if(FaultFilter_IsPower(record))
    {
        if(record->fs_detect_id.cabin_or_pack_id == SYS_MAIN_POWER_KEY_ID)
        {
            if(state == DISCONNECT) sprintf((char *)buffer, "\xD6\xF7\xB5\xE7\xB9\xCA\xD5\xCF");
            else sprintf((char *)buffer, "\xD6\xF7\xB5\xE7\xBB\xD6\xB8\xB4");
        }
        else
        {
            if(state == DISCONNECT) sprintf((char *)buffer, "\xB1\xB8\xB5\xE7\xB9\xCA\xD5\xCF");
            else if(state == SHORTCIRCUIT) sprintf((char *)buffer, "\xB1\xB8\xB5\xE7\xB6\xCC\xC2\xB7");
            else sprintf((char *)buffer, "\xB1\xB8\xB5\xE7\xBB\xD6\xB8\xB4");
        }
        return;
    }
    switch(state)
    {
        case DISCONNECT: sprintf((char *)buffer, "\xC9\xE8\xB1\xB8\xB5\xF4\xCF\xDF"); break;
        case DIS_RECOVERY: sprintf((char *)buffer, "\xB5\xF4\xCF\xDF\xBB\xD6\xB8\xB4"); break;
        case SHORTCIRCUIT: sprintf((char *)buffer, "\xC9\xE8\xB1\xB8\xB6\xCC\xC2\xB7"); break;
        case SHO_RECOVERY: sprintf((char *)buffer, "\xB6\xCC\xC2\xB7\xBB\xD6\xB8\xB4"); break;
        case RS485_TEMP_SENSOR_FAULT:
        case LOOP1_TEMP_SENSOR_FAULT: sprintf((char *)buffer, "\xCE\xC2\xB6\xC8\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"); break;
        case RS485_TEMP_SENSOR_RECOVERY:
        case LOOP1_TEMP_SENSOR_RECOVERY: sprintf((char *)buffer, "\xCE\xC2\xB6\xC8\xB4\xAB\xB8\xD0\xC6\xF7\xBB\xD6\xB8\xB4"); break;
        case RS485_SMOKE_POLLUTION_FAULT:
        case LOOP1_SMOKE_POLLUTION_FAULT: sprintf((char *)buffer, "\xD1\xCC\xCE\xED\xCE\xDB\xC8\xBE\xB9\xCA\xD5\xCF"); break;
        case RS485_SMOKE_POLLUTION_RECOVERY:
        case LOOP1_SMOKE_POLLUTION_RECOVERY: sprintf((char *)buffer, "\xD1\xCC\xCE\xED\xCE\xDB\xC8\xBE\xBB\xD6\xB8\xB4"); break;
        case LOOP1_SMOKE_SENSOR_FAULT: sprintf((char *)buffer, "\xD1\xCC\xCE\xED\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"); break;
        case LOOP1_SMOKE_SENSOR_RECOVERY: sprintf((char *)buffer, "\xD1\xCC\xCE\xED\xB4\xAB\xB8\xD0\xC6\xF7\xBB\xD6\xB8\xB4"); break;
        case RS485_CO_SENSOR_FAULT: sprintf((char *)buffer, "CO\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"); break;
        case RS485_CO_SENSOR_RECOVERY: sprintf((char *)buffer, "CO\xB4\xAB\xB8\xD0\xC6\xF7\xBB\xD6\xB8\xB4"); break;
        case RS485_H2_SENSOR_FAULT: sprintf((char *)buffer, "H2\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"); break;
        case RS485_H2_SENSOR_RECOVERY: sprintf((char *)buffer, "H2\xB4\xAB\xB8\xD0\xC6\xF7\xBB\xD6\xB8\xB4"); break;
        case RS485_VOC_SENSOR_FAULT: sprintf((char *)buffer, "VOC\xB4\xAB\xB8\xD0\xC6\xF7\xB9\xCA\xD5\xCF"); break;
...
static void FaultFilter_ShowRecord(uint8_t row, uint16_t serial, const FlashSaveDetectFault_t *record)
{
    uint8_t buffer[64] = {0};
    FlashSaveTime_t time_value = {0};
    SetTextInt32(FAULT_FILTER_SCREEN_ID, g_fault_serial_ids[row], serial, 0U, 1U);
    FaultFilter_FormatDevice(record, buffer);
    SetTextValue(FAULT_FILTER_SCREEN_ID, g_fault_device_ids[row], buffer);
    getFlashTime_Plus(record->fs_time_buff, &time_value);
    sprintf((char *)buffer, "%04u\xC4\xEA%02u\xD4\xC2%02u\xC8\xD5 %02u\xCA\xB1%02u\xB7\xD6%02u\xC3\xEB",
            time_value.years + 2000U, time_value.months, time_value.days,
            time_value.hours, time_value.minute, time_value.second);
    SetTextValue(FAULT_FILTER_SCREEN_ID, g_fault_time_ids[row], buffer);
    FaultFilter_FormatState(record, buffer);
...
static uint8_t FaultFilter_ScanAndDisplay(void)
{
    ...
    uint16_t sector;
    uint16_t matched = 0U;
    uint16_t displayed = 0U;
    uint16_t skip = (uint16_t)((g_fault_filter.page - 1U) * FAULT_FILTER_ROWS);

    FaultFilter_ClearRows();
    if(total < 0)
    {
        SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_READ_FAILED);
        return 0U;
    }
    sector_count = (uint16_t)(((uint16_t)total + FAULT_FILTER_RECORDS_PER_SECTOR - 1U) /
                              FAULT_FILTER_RECORDS_PER_SECTOR);
    if(sector_count > FAULT_FILTER_MAX_SECTORS) sector_count = FAULT_FILTER_MAX_SECTORS;
    for(sector = sector_count; sector > 0U; sector--)
    {
        uint16_t sector_index = (uint16_t)(sector - 1U);
        uint16_t first_index = (uint16_t)(sector_index * FAULT_FILTER_RECORDS_PER_SECTOR);
        uint16_t end_index = (uint16_t)(first_index + FAULT_FILTER_RECORDS_PER_SECTOR);
        uint16_t absolute;
        if(end_index > (uint16_t)total) end_index = (uint16_t)total;
        if(BspReadFlashData(FAULT_FLASH_SAVE, g_history_sector_cache.byte_buff, (uint8_t)sector_index) < 0)
        {
            SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_READ_FAILED);
            return 0U;
        }
        for(absolute = end_index; absolute > first_index; absolute--)
        {
            FlashSaveDetectFault_t *record = &g_history_sector_cache.fs_sys_fault[absolute - first_index - 1U];
            if(FaultFilter_Match(record))
            {
                if(matched >= skip && displayed < FAULT_FILTER_ROWS)
                {
                    FaultFilter_ShowRecord(displayed, (uint16_t)(matched + 1U), record);
                    displayed++;
                }
                matched++;
            }
    ...
    g_fault_filter.matched_count = matched;
    SetTextInt32(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_PAGE_ID, g_fault_filter.page, 0U, 1U);
    SetTextInt32(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_TOTAL_ID, matched, 0U, 1U);
    SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID,
...
static void FaultFilter_NotifyScreen(uint16_t screen_id)
{
    ...
    memset(&g_fault_filter, 0, sizeof(g_fault_filter));
    g_fault_filter.page = 1U;
    strcpy(g_fault_filter.date_text, "YYYYMMDD.YYYYMMDD");
    SetTextValue(FAULT_FILTER_SCREEN_ID, 2U, (uint8_t *)FAULT_TEXT_ALL_LOOPS);
    SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_DATE_FAULT_TEXT_ID, (uint8_t *)"YYYYMMDD.YYYYMMDD");
    SetTextValue(FAULT_FILTER_SCREEN_ID, 52U, (uint8_t *)FAULT_TEXT_ALL_FAULTS);
...
static void FaultFilter_NotifyMenu(uint16_t screen_id, uint16_t control_id, uint8_t item, uint8_t state)
{
    static const uint8_t *loop_text[] = {(const uint8_t *)FAULT_TEXT_ALL_LOOPS, (const uint8_t *)FAULT_TEXT_LOOP1,
                                         (const uint8_t *)FAULT_TEXT_LOOP2, (const uint8_t *)FAULT_TEXT_LOOP3,
                                         (const uint8_t *)FAULT_TEXT_SYSTEM};
    static const uint8_t *type_text[] = {(const uint8_t *)FAULT_TEXT_ALL_FAULTS, (const uint8_t *)FAULT_TEXT_POWER_FAULT,
                                         (const uint8_t *)FAULT_TEXT_COMM_FAULT, (const uint8_t *)FAULT_TEXT_SENSOR_FAULT,
                                         (const uint8_t *)FAULT_TEXT_OTHER_FAULT};
    if(screen_id != FAULT_FILTER_SCREEN_ID || state != 1U) return;
    if(control_id == 3U && item < 5U)
    {
        g_fault_filter.loop_filter = item;
        SetTextValue(FAULT_FILTER_SCREEN_ID, 2U, (uint8_t *)loop_text[item]);
        FaultFilter_ResetResult();
    }
    else if(control_id == 53U && item < 5U)
    {
        g_fault_filter.type_filter = item;
        SetTextValue(FAULT_FILTER_SCREEN_ID, 52U, (uint8_t *)type_text[item]);
        FaultFilter_ResetResult();
    }
}
...
static void FaultFilter_NotifyText(uint16_t screen_id, uint16_t control_id, const uint8_t *text)
{
    ...
    strncpy(g_fault_filter.date_text, (const char *)text, sizeof(g_fault_filter.date_text) - 1U);
    g_fault_filter.date_text[sizeof(g_fault_filter.date_text) - 1U] = '\0';
    FaultFilter_ResetResult();
...
static void FaultFilter_NotifyButton(uint16_t screen_id, uint16_t control_id, uint8_t state)
{
    ...
    {
        FaultFilter_ResetResult();
        if(!FaultFilter_ValidateDate()) return;
        SetTextValue(FAULT_FILTER_SCREEN_ID, FAULT_FILTER_MESSAGE_ID, (uint8_t *)FAULT_TEXT_QUERYING);
        g_fault_filter.query_valid = FaultFilter_ScanAndDisplay();
        return;
    }
    if(!g_fault_filter.query_valid) return;
    total_pages = (uint16_t)((g_fault_filter.matched_count + FAULT_FILTER_ROWS - 1U) / FAULT_FILTER_ROWS);
    if(total_pages == 0U) total_pages = 1U;
    if(control_id == 59U && g_fault_filter.page > 1U)
    {
        g_fault_filter.page--;
        (void)FaultFilter_ScanAndDisplay();
    }
    else if(control_id == 60U && g_fault_filter.page < total_pages)
    {
        g_fault_filter.page++;
        (void)FaultFilter_ScanAndDisplay();
...
/* HMI统一入口：根据画面ID分派到报警或故障筛选，保持上层调用简单。 */
...
void HistoryFilter_NotifyMenu(uint16_t screen_id, uint16_t control_id, uint8_t item, uint8_t state)
{
    ...
    FaultFilter_NotifyMenu(screen_id, control_id, item, state);
}
