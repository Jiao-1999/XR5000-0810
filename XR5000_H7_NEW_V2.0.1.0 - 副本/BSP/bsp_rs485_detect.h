...
void     RS485Detect_InjectSensorState(uint8_t addr, uint8_t sensor_idx, uint8_t state); /* 注入传感器状态(测试用) */
...
uint16_t RS485Detect_GetNationalCode(uint8_t addr);              /* 获取设备国标设备类型码(供联动逻辑显示用) */
