...
uint16_t RS485Detect_GetNationalTypeCode(uint8_t addr);           /* 获取设备实际返回并保存的0x000D国标类型码 */
uint16_t RS485Detect_GetProductCode(uint8_t addr);                /* 获取设备实际返回并保存的0x000E内部产品码 */
...
uint16_t RS485Detect_GetNationalCode(uint8_t addr);              /* 获取国标设备类型码(供逻辑屏显示) */
#endif
